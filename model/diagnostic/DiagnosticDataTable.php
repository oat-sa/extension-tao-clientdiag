...
//    public function getDiagnosticConfig(core_kernel_classes_Resource $testCenter)
    {
...
//        $config['storeParams'] = ['testCenter' => $testCenter->getUri()];
     ...
     */
    public static function getDiagnostic($id)
    {
...
//            }
     ...
     * @throws \common_exception_NoImplementation
     */
//    public function getDiagnostics(core_kernel_classes_Resource $testCenter, $options = array())
    public function getDiagnostics($options = array())
    {
        $storageService = ServiceManager::getServiceManager()->get(Storage::SERVICE_ID);
        if ($storageService instanceof PaginatedSqlStorage) {
//            $options[DataTableHelper::OPTION_FILTER] = [PaginatedSqlStorage::DIAGNOSTIC_TEST_CENTER => $testCenter->getUri()];
            return $this->paginate($storageService, $options, function($data) {
     ...
     * @throws \common_exception_NoImplementation
     */
    public static function removeDiagnostic($id)
    {
        $storageService = ServiceManager::getServiceManager()->get(Storage::SERVICE_ID);
        if ($storageService instanceof PaginatedStorage) {
            $ids = $id ? $id : [];
