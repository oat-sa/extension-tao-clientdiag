...
function(c){return"%"+("00"+c.charCodeAt(0).toString(16)).slice(-2)}).join(""))}}}),define("taoClientDiagnostic/tools/message",["jquery","ui/feedback"],function($,feedback){"use strict";return function(container){var $feedbackBox=$(container);$feedbackBox.data("error")&&feedback().error($feedbackBox.data("error")),$feedbackBox.data("message")&&feedback().error($feedbackBox.data("message"))}}),define("taoClientDiagnostic/controller/Authenticator/login",["jquery","taoClientDiagnostic/tools/message"],function($,showMessage){"use strict";return{start:function(){showMessage("#feedback-box")}}}),function(global,factory){
"object"==typeof exports&&"undefined"!=typeof module?module.exports=factory(require("lodash"),require("core/promise")):"function"==typeof define&&define.amd?define("core/moduleLoader",["lodash","core/promise"],factory):(global=global||self)["core/moduleLoader"]=factory(global._,global["core/promise"])}(this,function(_,Promise){"use strict";return _=_&&_.hasOwnProperty("default")?_.default:_,Promise=Promise&&Promise.hasOwnProperty("default")?Promise.default:Promise,function(requiredModules,validate,specs){var loaded={},modules={},excludes=[],bundles=[],loader={addList:function(moduleList){return _.forEach(moduleList,this.add,this),this},add:function(def){if(
_.isEmpty(def.module)||!_.isString(def.module))throw new TypeError("An AMD module must be defined");if(_.isEmpty(def.category)||!_.isString(def.category))throw new TypeError("Providers must belong to a category");return modules[def.category]=modules[def.category]||[],_.isNumber(def.position)?modules[def.category][def.position]=def.module:"prepend"===def.position||"before"===def.position?modules[def.category].unshift(def.module):modules[def.category].push(def.module),def.bundle&&!_.contains(bundles,def.bundle)&&bundles.push(def.bundle),this},append:function(def){return this.add(_.merge({position:"append"},def))},prepend:function(def){return this.add(_.merge({
position:"prepend"},def))},remove:function(module){return excludes.push(module),this},load:function(loadBundles){var self=this,dependencies=_(modules).values().flatten().uniq().difference(excludes).value(),loadModules=function(amdModules){return _.isArray(amdModules)&&amdModules.length?new Promise(function(resolve,reject){require(amdModules,function(){resolve([].slice.call(arguments))},reject)}):Promise.resolve()};return loadModules(loadBundles?bundles:[]).then(function(){return loadModules(dependencies)}).then(function(loadedModules){return _.forEach(dependencies,function(dependency,index){var module=loadedModules[index],category=_.findKey(modules,function(
val){return _.contains(val,dependency)});if(!validate(module))throw new TypeError("Invalid module");_.isString(category)&&(loaded[category]=loaded[category]||[],loaded[category].push(module))}),self.getModules()})},getModules:function(category){return _.isString(category)?loaded[category]||[]:_(loaded).values().flatten().uniq().value()},getCategories:function(){return _.keys(loaded)}};return validate=_.isFunction(validate)?validate:_.isPlainObject,_.forEach(requiredModules,function(moduleList,category){if(_.isEmpty(category)||!_.isString(category))throw new TypeError("Modules must belong to a category");if(!_.isArray(moduleList))throw new TypeError(
"A list of modules must be an array");if(!_.all(moduleList,validate))throw new TypeError("The list does not contain valid modules");loaded[category]?loaded[category]=loaded[category].concat(moduleList):loaded[category]=moduleList}),specs&&_(specs).functions().forEach(function(method){loader[method]=function(){return specs[method].apply(loader,[].slice.call(arguments))}}),loader}}),define("tpl!taoClientDiagnostic/component/diagnostic/tpl/component",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],data=data||{};var helper,options,buffer="",helperMissing=(helpers=this.merge(helpers
,Handlebars.helpers)).helperMissing;return buffer+='<div class="diagnostics-content-area">\n    <h1>'+(0,this.escapeExpression)((options={hash:{},data:data},(helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"Diagnostic Tool",options):helperMissing.call(depth0,"__","Diagnostic Tool",options)))+"</h1>\n</div>"})}),define("taoClientDiagnostic/component/diagnostic/diagnosticLoader",["lodash","ui/component","core/moduleLoader","context","module","tpl!taoClientDiagnostic/component/diagnostic/tpl/component","layout/loading-bar"],function(_,componentFactory,moduleLoader,context,module,componentTpl,loadingBar){"use strict";var defaults={};return function(
container,config){var component=componentFactory({},defaults).setTemplate(componentTpl).on("init",function(){this.render(container)}).on("render",function(){var self=this,moduleConfig=module.config(),identifiers=_.keys(moduleConfig.diagnostics);moduleLoader({},_.isFunction).addList(moduleConfig.diagnostics).load(context.bundle).then(function(factories){var factoryConfig,factoryName,componentConfig=self.getConfig();_.forEach(factories,function(factory,index){factoryName=identifiers[index],(factoryConfig=componentConfig[factoryName]).controller=componentConfig.controller,factory(self.getElement(),factoryConfig).on("render",function(){
factoryConfig.autoStart&&this.run()}).on("start",function(){loadingBar.start()}).on("end",function(){loadingBar.stop()})}),self.trigger("ready")})});return _.defer(function(){component.init(config)}),component}}),define("taoClientDiagnostic/controller/CompatibilityChecker/diagnostics",["jquery","ui/feedback","taoClientDiagnostic/tools/message","taoClientDiagnostic/component/diagnostic/diagnosticLoader"],function($,feedback,showMessage,diagnosticFactory){"use strict";return{start:function(config){var $contentArea=$(".content");showMessage("#feedback-box"),diagnosticFactory($contentArea,config)}}}),function(global,factory){
...
;var stack1,helper,options,buffer="",functionType="function",escapeExpression=this.escapeExpression,self=this,helperMissing=helpers.helperMissing;function program2(depth0,data){var stack1,helper,buffer="";return buffer+="<p>",stack1=(helper=helpers.diagInstructions)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.diagInstructions)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+"</p>"}function program9(depth0,data){var helper,options,buffer="";return buffer+='\n        <p>\n            <label for="school_number">'+escapeExpression((options={hash:{},data:data},(
helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"School number:",options):helperMissing.call(depth0,"__","School number:",options)))+'</label>\n            <input type="text" data-control="school_number" id="school_number" name="school_number" maxlength="10" placeholder="'+escapeExpression((options={hash:{},data:data},(helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"School number",options):helperMissing.call(depth0,"__","School number",options)))+'" />\n        </p>\n        <p>\n            <label for="school_pin">'+escapeExpression((options={hash:{},data:data},(helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"School PIN:",
options):helperMissing.call(depth0,"__","School PIN:",options)))+'</label>\n            <input type="text" data-control="school_pin" id="school_pin" name="school_pin" maxlength="4" placeholder="'+escapeExpression((options={hash:{},data:data},(helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"School PIN code (4 digits)",options):helperMissing.call(depth0,"__","School PIN code (4 digits)",options)))+'" />\n        </p>\n            '}function program11(depth0,data){var helper,options,buffer="";return buffer+='\n        <p>\n            <label for="school_name">'+escapeExpression((options={hash:{},data:data},(helper=helpers.__||depth0&&depth0.__
)?helper.call(depth0,"School name:",options):helperMissing.call(depth0,"__","School name:",options)))+'</label>\n            <input type="text" data-control="school_name" id="school_name" name="school_name" maxlength="255" placeholder="'+escapeExpression((options={hash:{},data:data},(helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"School name",options):helperMissing.call(depth0,"__","School name",options)))+'" />\n        </p>\n            '}return buffer+='<div class="diagnostics-main-area">\n\n    <h1>',stack1=(helper=helpers.title)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.title)===functionType?helper.call(depth0,{
hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'</h1>\n\n    <div class="intro">\n        ',!(stack1=helpers.with.call(depth0,depth0&&depth0.configurableText,{hash:{},inverse:self.noop,fn:self.program(1,function(depth0,data){var stack1,buffer="";return buffer+="\n            ",!(stack1=helpers.if.call(depth0,depth0&&depth0.diagInstructions,{hash:{},inverse:self.noop,fn:self.program(2,program2,data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+="\n        "},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+="\n        ",!(stack1=helpers.if.call(depth0,depth0&&depth0.header,{hash:{},inverse:self.noop,fn:self.program(4,function(depth0,
data){var stack1,helper,buffer="";return buffer+="<p>",stack1=(helper=helpers.header)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.header)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+"</p>"},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+="\n        ",!(stack1=helpers.if.call(depth0,depth0&&depth0.info,{hash:{},inverse:self.noop,fn:self.program(6,function(depth0,data){var stack1,helper,buffer="";return buffer+="<p>",stack1=(helper=helpers.info)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.info)===functionType?helper.call(depth0,{hash:{},data:data}
):helper,buffer+=escapeExpression(stack1)+"</p>"},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+="\n        ",!(stack1=helpers.if.call(depth0,depth0&&depth0.requireSchoolName,{hash:{},inverse:self.noop,fn:self.program(8,function(depth0,data){var stack1,buffer="";return buffer+="\n            ",!(stack1=helpers.if.call(depth0,depth0&&depth0.validateSchoolName,{hash:{},inverse:self.program(11,program11,data),fn:self.program(9,program9,data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+="\n        "},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+='\n        <p>\n            <label for="workstation">'+escapeExpression((options={hash:{},
data:data},(helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"Workstation:",options):helperMissing.call(depth0,"__","Workstation:",options)))+'</label>\n            <input type="text" data-control="workstation" id="workstation" name="workstation" maxlength="64" placeholder="'+escapeExpression((options={hash:{},data:data},(helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"Workstation name",options):helperMissing.call(depth0,"__","Workstation name",options)))+'" />\n        </p>\n    </div>\n\n    <div class="clearfix">\n        <button data-action="test-launcher" class="btn-info small rgt">',stack1=(helper=helpers.button)?helper.call(depth0,{
hash:{},data:data}):typeof(helper=depth0&&depth0.button)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'</button>\n    </div>\n\n    <ul class="plain results"></ul>\n\n    <div class="status">\n        <h2></h2>\n    </div>\n\n</div>'})}),define("taoClientDiagnostic/controller/Diagnostic/diagnostic",["module","jquery","i18n","helpers","layout/loading-bar","ui/actionbar","ui/feedback","taoClientDiagnostic/tools/diagnostic/diagnostic","tpl!taoClientDiagnostic/templates/diagnostic/main"],function(module,$,__,helpers,loadingBar,actionbar,feedback,diagnosticFactory,diagnosticTpl){"use strict"
;var taoDiagnosticRunnerCtlr={start:function(){var $container=$(".diagnostic-runner"),extension=$container.data("extension")||"taoClientDiagnostic",$list=$container.find(".list"),$panel=$(".panel"),extensionConfig=$container.data("config")||{},config=extensionConfig.diagnostic||extensionConfig,indexUrl=helpers._url("index","Diagnostic",extension),workstationUrl=helpers._url("workstation","DiagnosticChecker",extension),buttons=[],moduleConfig=module.config()||{};function installTester(workstation){diagnosticFactory($list,config).setTemplate(diagnosticTpl).on("render",function(){var self=this;this.controls.$workstation=this.getElement().find(
'[data-control="workstation"]').on("keypress",function(e){13===e.which&&(e.preventDefault(),self.run())}).val(workstation)}).on("start",function(){this.config.storeParams=this.config.storeParams||{},this.config.storeParams.workstation=this.controls.$workstation.val(),this.controls.$workstation.prop("disabled",!0),loadingBar.start()}).on("end",function(){this.controls.$workstation.removeProp("disabled"),loadingBar.stop()}).on("render",function(){loadingBar.stop(),config.autoStart&&this.run()})}config.configurableText=moduleConfig.configurableText||{},buttons.push({id:"back",icon:"step-backward",title:__("Return to the list"),label:__(
"List of readiness checks"),action:function(){window.location.href=indexUrl}}),window.self===window.top&&buttons.push({id:"exitButton",icon:"close",title:__("Exit"),label:__("Exit"),action:function(){window.self.close()}}),actionbar({renderTo:$panel,buttons:buttons}),$.get(workstationUrl,"json").done(function(data){installTester(data&&data.workstation)}).fail(function(){feedback().error(__("Unable to get the workstation name!")),installTester()})}};return loadingBar.start(),taoDiagnosticRunnerCtlr}),function(e,t,i){"use strict";"function"==typeof define&&define.amd?define("taoClientDiagnostic/lib/fingerprint/fingerprint2",i
...
"Technical","Teletype","Telugu Sangam MN","Tempus Sans ITC","Terminal","Thonburi","Traditional Arabic","Trajan","TRAJAN PRO","Tristan","Tubular","Tunga","Tw Cen MT","Tw Cen MT Condensed","Tw Cen MT Condensed Extra Bold","TypoUpright BT","Unicorn","Univers","Univers CE 55 Medium","Univers Condensed","Utsaah","Vagabond","Vani","Vijaya","Viner Hand ITC","VisualUI","Vivaldi","Vladimir Script","Vrinda","Westminster","WHITNEY","Wide Latin","ZapfEllipt BT","ZapfHumnst BT","ZapfHumnst Dm BT","Zapfino","Zurich BlkEx BT","Zurich Ex BT","ZWAdobeF"])),r=r.concat(i.options.userDefinedFonts);function g(){var e=document.createElement("span")
;return e.style.position="absolute",e.style.left="-9999px",e.style.fontSize="72px",e.style.lineHeight="normal",e.innerHTML="mmmmmmmmmmlli",e}function p(e,t){var i=g();return i.style.fontFamily="'"+e+"',"+t,i}function T(e){for(var t=!1,i=0;i<a.length;i++)if(t=e[i].offsetWidth!==c[a[i]]||e[i].offsetHeight!==d[a[i]])return t;return t}var l=document.getElementsByTagName("body")[0],h=document.createElement("div"),u=document.createElement("div"),c={},d={},S=function(){for(var e=[],t=0,i=a.length;t<i;t++){var r=g();r.style.fontFamily=a[t],h.appendChild(r),e.push(r)}return e}();l.appendChild(h);for(var x=0,v=a.length;x<v;x++)c[a[x]]=S[x].offsetWidth,
d[a[x]]=S[x].offsetHeight;var E=function(){for(var e={},t=0,i=r.length;t<i;t++){for(var n=[],o=0,s=a.length;o<s;o++){var l=p(r[t],a[o]);u.appendChild(l),n.push(l)}e[r[t]]=n}return e}();l.appendChild(u);for(var M=[],A=0,y=r.length;A<y;A++)T(E[r[A]])&&M.push(r[A]);l.removeChild(u),l.removeChild(h),e.push({key:"js_fonts",value:M}),t(e)},1)},pluginsKey:function(e){return this.options.excludePlugins||(this.isIE()?this.options.excludeIEPlugins||e.push({key:"ie_plugins",value:this.getIEPlugins()}):e.push({key:"regular_plugins",value:this.getRegularPlugins()})),e},getRegularPlugins:function(){for(var e=[],t=0,i=navigator.plugins.length;t<i;t++)e.push(
navigator.plugins[t]);return this.pluginsShouldBeSorted()&&(e=e.sort(function(e,t){return e.name>t.name?1:e.name<t.name?-1:0})),this.map(e,function(e){var t=this.map(e,function(e){return[e.type,e.suffixes].join("~")}).join(",");return[e.name,e.description,t].join("::")},this)},getIEPlugins:function(){var e=[];if(Object.getOwnPropertyDescriptor&&Object.getOwnPropertyDescriptor(window,"ActiveXObject")||"ActiveXObject"in window){e=this.map(["AcroPDF.PDF","Adodb.Stream","AgControl.AgControl","DevalVRXCtrl.DevalVRXCtrl.1","MacromediaFlashPaper.MacromediaFlashPaper","Msxml2.DOMDocument","Msxml2.XMLHTTP","PDF.PdfCtrl","QuickTime.QuickTime",
"QuickTimeCheckObject.QuickTimeCheck.1","RealPlayer","RealPlayer.RealPlayer(tm) ActiveX Control (32-bit)","RealVideo.RealVideo(tm) ActiveX Control (32-bit)","Scripting.Dictionary","SWCtl.SWCtl","Shell.UIHelper","ShockwaveFlash.ShockwaveFlash","Skype.Detection","TDCCtl.TDCCtl","WMPlayer.OCX","rmocx.RealPlayer G2 Control","rmocx.RealPlayer G2 Control.1"],function(e){try{return new ActiveXObject(e),e}catch(t){return null}})}return navigator.plugins&&(e=e.concat(this.getRegularPlugins())),e},pluginsShouldBeSorted:function(){for(var e=!1,t=0,i=this.options.sortPluginsFor.length;t<i;t++){var a=this.options.sortPluginsFor[t];if(navigator.userAgent.match(a)){e=!0
...
getNavigatorCpuClass:function(){return navigator.cpuClass?navigator.cpuClass:"unknown"},getNavigatorPlatform:function(){return navigator.platform?navigator.platform:"unknown"},getDoNotTrack:function(){return navigator.doNotTrack?navigator.doNotTrack:navigator.msDoNotTrack?navigator.msDoNotTrack:window.doNotTrack?window.doNotTrack:"unknown"},getTouchSupport:function(){var e=0,t=!1;void 0!==navigator.maxTouchPoints?e=navigator.maxTouchPoints:void 0!==navigator.msMaxTouchPoints&&(e=navigator.msMaxTouchPoints);try{document.createEvent("TouchEvent"),t=!0}catch(i){}return[e,t,"ontouchstart"in window]},getCanvasFp:function(){var e=[],t=document.createElement(
"canvas");t.width=2e3,t.height=200,t.style.display="inline";var i=t.getContext("2d");return i.rect(0,0,10,10),i.rect(2,2,6,6),e.push("canvas winding:"+(!1===i.isPointInPath(5,5,"evenodd")?"yes":"no")),i.textBaseline="alphabetic",i.fillStyle="#f60",i.fillRect(125,1,62,20),i.fillStyle="#069",this.options.dontUseFakeFontInCanvas?i.font="11pt Arial":i.font="11pt no-real-font-123",i.fillText("Cwm fjordbank glyphs vext quiz, 😃",2,15),i.fillStyle="rgba(102, 204, 0, 0.2)",i.font="18pt Arial",i.fillText("Cwm fjordbank glyphs vext quiz, 😃",4,45),i.globalCompositeOperation="multiply",i.fillStyle="rgb(255,0,255)",i.beginPath(),i.arc(50,50,50,0,2*Math.PI,!0),
i.closePath(),i.fill(),i.fillStyle="rgb(0,255,255)",i.beginPath(),i.arc(100,50,50,0,2*Math.PI,!0),i.closePath(),i.fill(),i.fillStyle="rgb(255,255,0)",i.beginPath(),i.arc(75,100,50,0,2*Math.PI,!0),i.closePath(),i.fill(),i.fillStyle="rgb(255,0,255)",i.arc(75,75,75,0,2*Math.PI,!0),i.arc(75,75,25,0,2*Math.PI,!0),i.fill("evenodd"),e.push("canvas fp:"+t.toDataURL()),e.join("~")},getWebglFp:function(){var e,t=function(t){return e.clearColor(0,0,0,1),e.enable(e.DEPTH_TEST),e.depthFunc(e.LEQUAL),e.clear(e.COLOR_BUFFER_BIT|e.DEPTH_BUFFER_BIT),"["+t[0]+", "+t[1]+"]"};if(!(e=this.getWebglCanvas()))return null;var a=[],o=e.createBuffer();e.bindBuffer(e.ARRAY_BUFFER,o)
;var s=new Float32Array([-.2,-.9,0,.4,-.26,0,0,.732134444,0]);e.bufferData(e.ARRAY_BUFFER,s,e.STATIC_DRAW),o.itemSize=3,o.numItems=3;var l=e.createProgram(),h=e.createShader(e.VERTEX_SHADER);e.shaderSource(h,"attribute vec2 attrVertex;varying vec2 varyinTexCoordinate;uniform vec2 uniformOffset;void main(){varyinTexCoordinate=attrVertex+uniformOffset;gl_Position=vec4(attrVertex,0,1);}"),e.compileShader(h);var u=e.createShader(e.FRAGMENT_SHADER);e.shaderSource(u,"precision mediump float;varying vec2 varyinTexCoordinate;void main() {gl_FragColor=vec4(varyinTexCoordinate,0,1);}"),e.compileShader(u),e.attachShader(l,h),e.attachShader(l,u),e.linkProgram(l),
e.useProgram(l),l.vertexPosAttrib=e.getAttribLocation(l,"attrVertex"),l.offsetUniform=e.getUniformLocation(l,"uniformOffset"),e.enableVertexAttribArray(l.vertexPosArray),e.vertexAttribPointer(l.vertexPosAttrib,o.itemSize,e.FLOAT,!1,0,0),e.uniform2f(l.offsetUniform,1,1),e.drawArrays(e.TRIANGLE_STRIP,0,o.numItems),null!=e.canvas&&a.push(e.canvas.toDataURL()),a.push("extensions:"+e.getSupportedExtensions().join(";")),a.push("webgl aliased line width range:"+t(e.getParameter(e.ALIASED_LINE_WIDTH_RANGE))),a.push("webgl aliased point size range:"+t(e.getParameter(e.ALIASED_POINT_SIZE_RANGE))),a.push("webgl alpha bits:"+e.getParameter(e.ALPHA_BITS)),a.push(
"webgl antialiasing:"+(e.getContextAttributes().antialias?"yes":"no")),a.push("webgl blue bits:"+e.getParameter(e.BLUE_BITS)),a.push("webgl depth bits:"+e.getParameter(e.DEPTH_BITS)),a.push("webgl green bits:"+e.getParameter(e.GREEN_BITS)),a.push("webgl max anisotropy:"+function(e){var t,i=e.getExtension("EXT_texture_filter_anisotropic")||e.getExtension("WEBKIT_EXT_texture_filter_anisotropic")||e.getExtension("MOZ_EXT_texture_filter_anisotropic");return i?(0===(t=e.getParameter(i.MAX_TEXTURE_MAX_ANISOTROPY_EXT))&&(t=2),t):null}(e)),a.push("webgl max combined texture image units:"+e.getParameter(e.MAX_COMBINED_TEXTURE_IMAGE_UNITS)),a.push(
