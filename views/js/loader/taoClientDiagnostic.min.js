...
"use strict";return function(container){var $feedbackBox=$(container);$feedbackBox.data("error")&&feedback().error($feedbackBox.data("error")),$feedbackBox.data("message")&&feedback().error($feedbackBox.data("message"))}}),define("taoClientDiagnostic/controller/Authenticator/login",["jquery","taoClientDiagnostic/tools/message"],function($,showMessage){"use strict";return{start:function(){showMessage("#feedback-box")}}}),define("taoClientDiagnostic/tools/fixedDecimals",[],function(){"use strict";return function(value,decimals){var shift=Math.pow(10,Math.abs(decimals||1));return Math.round(Number(value)*shift)/shift||0}}),define(
"taoClientDiagnostic/tools/stats",["lodash","taoClientDiagnostic/tools/fixedDecimals"],function(_,fixedDecimals){"use strict";return function(list,fieldName,decimals){var variance,avg,med,middle,getValue,results,sum=0,sum2=0,min=Number.MAX_VALUE,max=0,count=0,values=[];return getValue=_.isFunction(fieldName)?fieldName:function(item){return item&&item[fieldName]||0},_.forEach(list,function(item){var value;void 0!==item&&(value=getValue(item),values.push(value),min=Math.min(min,value),max=Math.max(max,value),sum+=value,count++)}),avg=sum/(count||1),_.forEach(values,function(value){var diff=value-avg;sum2+=diff*diff}),variance=1<count?Math.sqrt(sum2/(count-1)
):0,values.sort(),middle=count/2,med=(values[Math.floor(middle)]+values[Math.ceil(middle)])/2,results={min:min,max:max,sum:sum,count:count,average:avg,median:med,variance:variance},decimals&&_.forEach(results,function(value,key){results[key]=fixedDecimals(value,decimals)}),results.values=list,results}}),define("taoClientDiagnostic/tools/getConfig",["lodash"],function(_){"use strict";return function(config,defaults){return _(config||{}).omit(function(value){return null==value}).defaults(defaults||{}).value()}}),define("taoClientDiagnostic/tools/getLabels",["lodash"],function(_){"use strict";return function(messages,level){return messages=messages||{},
_.isArray(messages)||(messages=[messages]),messages[level=Math.min(Math.max(parseInt(level,10)||0,1),messages.length||1)-1]||{}}}),define("taoClientDiagnostic/tools/getStatus",["lodash"],function(_){"use strict";return function(percentage,thresholds,opts){var len,feedback,i,step,options=opts||{},testPercentage=Math.max(0,Math.min(100,Math.round(parseInt(percentage,10)||0))),status={percentage:testPercentage,globalPercentage:options.minimumGlobalPercentage?Math.max(testPercentage,options.minimumGlobalPercentage):testPercentage,quality:{}};if(thresholds){for(_.isArray(thresholds)||(thresholds=[thresholds]),len=thresholds.length,i=0;i<len&&((step=thresholds[i]
)&&(!step.threshold||status.percentage>=step.threshold));i++)feedback=step;feedback&&(status.feedback=_.clone(feedback))}return status}}),define("taoClientDiagnostic/tools/performances/tester",["jquery","lodash","i18n","async","context","helpers","taoClientDiagnostic/tools/stats","taoQtiItem/qtiItem/core/Loader","taoQtiItem/qtiCommonRenderer/renderers/Renderer","taoClientDiagnostic/tools/getConfig","taoClientDiagnostic/tools/getLabels","taoClientDiagnostic/tools/getStatus"],function($,_,__,async,context,helpers,stats,Loader,Renderer,getConfig,getLabels,getStatus){"use strict";var _second=1e3,_defaultTimeout=30*_second,_defaultSamples=[
"taoClientDiagnostic/tools/performances/data/sample1/","taoClientDiagnostic/tools/performances/data/sample2/","taoClientDiagnostic/tools/performances/data/sample3/"],_defaults={id:"performances",optimal:.025,threshold:.25},_thresholds=[{threshold:0,message:__("Very slow performances"),type:"error"},{threshold:33,message:__("Average performances"),type:"warning"},{threshold:66,message:__("Good performances"),type:"success"}],_messages=[{title:__("Workstation performances"),status:__("Checking the performances..."),performancesMin:__("Minimum rendering time"),performancesMax:__("Maximum rendering time"),performancesAverage:__("Average rendering time")}]
;function loadItem(data,done){var qtiJsonFile=data.url+"qti.json",extension=data.url.split("/")[0],fullpath=require.s.contexts._.config.paths[extension],baseUrl=data.url.replace(extension,fullpath),loader=new Loader,renderer=new Renderer({baseUrl:baseUrl});renderer.getAssetManager&&renderer.getAssetManager().setData("baseUrl",baseUrl),require(["json!"+qtiJsonFile],function(itemData){loader.loadItemData(itemData,function(item){renderer.load(function(){var $container,duration,start,result;start=window.performance.now(),item.setRenderer(this),($container=$("<div>").appendTo("body")).append(item.render()),item.postRender(),$container.remove(),duration=(
window.performance.now()-start)/_second,result={id:data.id,url:data.url,duration:duration},done(null,result)},this.getLoadedClasses())})})}return function(config){var initConfig=getConfig(config,_defaults),labels=getLabels(_messages,initConfig.level),idx=0,_samples=_.map(!_.isEmpty(initConfig.samples)&&initConfig.samples||_defaultSamples,function(sample){return{id:"sample"+ ++idx,url:sample,timeout:1e3*initConfig.timeout||_defaultTimeout,nb:initConfig.occurrences||10}});return _samples[0].nb++,{start:function(done){var tests=[],self=this;_.forEach(_samples,function(data){for(var cb=_.partial(loadItem,data),iterations=data.nb||1;iterations--;)tests.push(cb)})
,async.series(tests,function(err,measures){var results,status,summary;if(err&&!measures.length)throw err;measures.shift(),results=stats(measures,"duration",2),status=self.getFeedback(results.average),summary=self.getSummary(results),done(status,summary,results)})},get labels(){return labels},getSummary:function(results){return{performancesMin:{message:labels.performancesMin,value:results.min+" s"},performancesMax:{message:labels.performancesMax,value:results.max+" s"},performancesAverage:{message:labels.performancesAverage,value:results.average+" s"}}},getFeedback:function(result){var optimal=initConfig.optimal,range=Math.abs(optimal-initConfig.threshold),
status=getStatus((range+optimal-result)/range*100,_thresholds);return status.title=labels.title,status.id=initConfig.id,status}}}}),define("taoClientDiagnostic/tools/bandwidth/tester",["lodash","i18n","async","context","taoClientDiagnostic/tools/getConfig","taoClientDiagnostic/tools/getLabels","taoClientDiagnostic/tools/stats","taoClientDiagnostic/tools/fixedDecimals","taoClientDiagnostic/tools/getStatus"],function(_,__,async,context,getConfig,getLabels,stats,fixedDecimals,getStatus){"use strict";var _mega=1048576,_second=1e3,_thresholds=[{threshold:0,message:__("Low bandwidth"),type:"error"},{threshold:33,message:__("Average bandwidth"),type:"warning"},{
threshold:66,message:__("Good bandwidth"),type:"success"}],_defaults={id:"bandwidth",unit:.16,ideal:45,max:100,minimumGlobalPercentage:!1,feedbackThresholds:_thresholds,fallbackThreshold:.2},_downloadData={"10KB":{id:"10KB",file:"data/bin10KB.data",size:10240,timeout:_second,threshold:0,nb:10},"100KB":{id:"100KB",file:"data/bin100KB.data",size:102400,timeout:2*_second,threshold:0,nb:5},"1MB":{id:"1MB",file:"data/bin1MB.data",size:_mega,timeout:20*_second,threshold:0,nb:3}},_messages=[{title:__("Bandwidth"),status:__("Checking the bandwidth..."),legend:__("Number of simultaneous test takers the connection can handle"),bandwidthMin:__("Minimum bandwidth"),
bandwidthMax:__("Maximum bandwidth"),bandwidthAverage:__("Average bandwidth")},{title:__("Media intensive bandwidth"),status:__("Checking the media intensive bandwidth..."),legend:__("Number of simultaneous test takers the connection can handle with media intensive"),bandwidthMin:__("Minimum intensive bandwidth"),bandwidthMax:__("Maximum intensive bandwidth"),bandwidthAverage:__("Average intensive bandwidth")}];function download(data,cb){var start,end,timeoutId,url,request,self=this;if(data.threshold&&this.bandwidth<data.threshold)return cb("threshold");url=context.root_url+"taoClientDiagnostic/views/js/tools/bandwidth/"+data.file+"?"+Date.now(),(
...
var $container=$(".diagnostic-runner"),extension=$container.data("extension")||"taoClientDiagnostic",$list=$container.find(".list"),$panel=$(".panel"),config=$container.data("config"),indexUrl=helpers._url("index","Diagnostic",extension),workstationUrl=helpers._url("workstation","DiagnosticChecker",extension),buttons=[],moduleConfig=module.config()||{};function installTester(workstation){diagnosticFactory(config).setTemplate(diagnosticTpl).on("render",function(){var self=this;this.controls.$workstation=this.getElement().find('[data-control="workstation"]').on("keypress",function(e){13===e.which&&(e.preventDefault(),self.run())}).val(workstation)}).on("start"
,function(){this.config.storeParams=this.config.storeParams||{},this.config.storeParams.workstation=this.controls.$workstation.val(),this.controls.$workstation.prop("disabled",!0),loadingBar.start()}).on("end",function(){this.controls.$workstation.removeProp("disabled"),loadingBar.stop()}).on("render",function(){loadingBar.stop(),config.autoStart&&this.run()}).render($list)}config.configurableText=moduleConfig.configurableText||{},buttons.push({id:"back",icon:"step-backward",title:__("Return to the list"),label:__("List of readiness checks"),action:function(){window.location.href=indexUrl}}),window.self===window.top&&buttons.push({id:"exitButton",
icon:"close",title:__("Exit"),label:__("Exit"),action:function(){window.self.close()}}),actionbar({renderTo:$panel,buttons:buttons}),$.get(workstationUrl,"json").done(function(data){installTester(data&&data.workstation)}).fail(function(){feedback().error(__("Unable to get the workstation name!")),installTester()})}};return loadingBar.start(),taoDiagnosticRunnerCtlr}),function(e,t,i){"use strict";"function"==typeof define&&define.amd?define("taoClientDiagnostic/lib/fingerprint/fingerprint2",i):"undefined"!=typeof module&&module.exports?module.exports=i():t.exports?t.exports=i():t.Fingerprint2=i()}(0,this,function(){"use strict";var e=function(t){if(!(
this instanceof e))return new e(t);this.options=this.extend(t,{swfContainerId:"fingerprintjs2",swfPath:"flash/compiled/FontList.swf",detectScreenOrientation:!0,sortPluginsFor:[/palemoon/i],userDefinedFonts:[]}),this.nativeForEach=Array.prototype.forEach,this.nativeMap=Array.prototype.map};return e.prototype={extend:function(e,t){if(null==e)return t;for(var i in e)null!=e[i]&&t[i]!==e[i]&&(t[i]=e[i]);return t},get:function(e){var t=this,i={data:[],push:function(e){var i=e.key,a=e.value;"function"==typeof t.options.preprocessor&&(a=t.options.preprocessor(i,a)),this.data.push({key:i,value:a})}};i=this.userAgentKey(i),i=this.languageKey(i),i=this.colorDepthKey(i
