...
return request(url,null,"POST")},run:function(){var self=this,information=[],scores={},testers=[],customInput=self.getCustomInput(),doRun=function(){function doCheck(testerConfig,cb){var testerId=testerConfig.id;self.trigger("starttester",testerId),self.setState(testerId,!0),require([testerConfig.tester],function(testerFactory){var tester=testerFactory(getConfig(testerConfig,self.config),self);self.changeStatus(tester.labels.status),tester.start(function(status,details,results){var customMsg;testerConfig.customMsgKey&&(customMsg=self.getCustomMsg(testerConfig.customMsgKey),self.addCustomFeedbackMsg(status,customMsg)),_.forEach(details,function(info){information.push(info);
...
return buffer+="<p>",(helper=helpers.info)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.info,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+"</p>"}function program8(depth0,data){var helper,options,buffer="";return buffer+='\n        <p>\n            <label for="school">'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},helper?helper.call(depth0,"School:",options):helperMissing.call(depth0,"__","School:",options)))+'</label>\n            <input type="text" data-control="school" id="school" name="school" maxlength="255" placeholder="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,
