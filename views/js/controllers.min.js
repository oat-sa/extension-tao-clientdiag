...
}var $container=$(cssScope),$list=$container.find(".list"),$panel=$(".panel"),config=$container.data("config"),indexUrl=helpers._url("index","Diagnostic","taoClientDiagnostic"),workstationUrl=helpers._url("workstation","DiagnosticChecker","taoClientDiagnostic");actionbar({renderTo:$panel,buttons:[{id:"back",icon:"step-backward",title:__("Return to the list"),label:__("List of readiness checks"),action:function(){window.location.href=indexUrl}}]}),$.get(workstationUrl,"json").done(function(data){installTester(data&&data.workstation)}).fail(function(){feedback().error(__("Unable to get the workstation name!")),installTester()})}};return taoDiagnosticRunnerCtlr}),
define("taoClientDiagnostic/controller/Diagnostic/index",["jquery","i18n","helpers","layout/loading-bar","util/encode","ui/feedback","ui/dialog","taoClientDiagnostic/tools/diagnostic/status","ui/datatable"],function($,__,helpers,loadingBar,encode,feedback,dialog,statusFactory){"use strict";function formatNumber(number,digits){var nb=void 0===digits?2:Math.max(0,parseInt(digits,10)),factor=Math.pow(10,nb)||1;return Math.round(number*factor)/factor}var cssScope=".diagnostic-index";loadingBar.start();var taoDiagnosticCtlr={start:function(){function request(url,selection,message){selection&&selection.length&&(loadingBar.start(),$.ajax({url:url,data:{id:selection},
dataType:"json",type:"POST",error:function(){loadingBar.stop()}}).done(function(response){loadingBar.stop(),response&&response.success?(message&&feedback().success(message),$list.datatable("refresh")):feedback().error(__("Something went wrong ...")+"<br>"+encode.html(response.error),{encodeHtml:!1})}))}function remove(selection){request(removeUrl,selection,__("The readiness check result have been removed"))}var $container=$(cssScope),$list=$container.find(".list"),dataset=$container.data("set"),config=$container.data("config")||{},installedExtension=$container.data("installedextension")||!1,diagnosticUrl=helpers._url("diagnostic","Diagnostic","taoClientDiagnostic"),removeUrl=helpers._url("remove","Diagnostic","taoClientDiagnostic"),serviceUrl=helpers._url("diagnosticData","Diagnostic","taoClientDiagnostic"),performancesConfig=config.performances||{},performancesOptimal=performancesConfig.optimal,performancesRange=Math.abs(performancesOptimal-performancesConfig.threshold),diagnosticStatus=statusFactory(),tools=[],actions=[],model=[];
