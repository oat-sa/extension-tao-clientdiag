...
define("taoClientDiagnostic/controller/Authenticator/login",["jquery","taoClientDiagnostic/tools/message"],function($,showMessage){"use strict";return{start:function(){showMessage("#feedback-box")}}}),define("taoClientDiagnostic/tools/diagnostic/status",["jquery","lodash","i18n"],function($,_,__){"use strict";function diagnosticStatusFactory(){return{thresholds:{performances:performancesThresholds,bandwidth:bandwidthThresholds,upload:uploadThresholds,summary:summaryThresholds},getThresholds:function(name){return this.thresholds[name]},getStatus:function(percentage,thresholds){var len,feedback,i,step,status
;if(percentage=Math.max(0,Math.min(100,Math.round(parseInt(percentage,10)))),_.isString(thresholds)&&(thresholds=this.getThresholds(thresholds)),thresholds)for(_.isArray(thresholds)||(thresholds=[thresholds]),len=thresholds.length,feedback=thresholds[0],i=1;i<len&&((step=thresholds[i])&&percentage>=step.threshold);i++)feedback=step;return status={percentage:percentage,quality:{}},feedback&&(status.feedback=_.clone(feedback)),status}}}var performancesThresholds=[{threshold:0,message:__("Very slow performances"),type:"error"},{threshold:33,message:__("Average performances"),type:"warning"},{threshold:66,message:__("Good performances"),type:"success"
}],bandwidthThresholds=[{threshold:0,message:__("Low bandwidth"),type:"error"},{threshold:33,message:__("Average bandwidth"),type:"warning"},{threshold:66,message:__("Good bandwidth"),type:"success"}],uploadThresholds=[{threshold:0,message:__("Low upload speed"),type:"error"},{threshold:33,message:__("Average upload speed"),type:"warning"},{threshold:66,message:__("Good upload speed"),type:"success"}],summaryThresholds=[{threshold:0,message:__("Your system requires a compatibility update, please contact your system administrator."),type:"error"},{threshold:33,message:__("Your system is not optimal, please contact your system administrator."),type:"warning"
},{threshold:66,message:__("Your system is fully compliant."),type:"success"}];return diagnosticStatusFactory}),define("taoClientDiagnostic/tools/fixedDecimals",[],function(){"use strict";return function(value,decimals){var shift=Math.pow(10,Math.abs(decimals||1));return Math.round(Number(value)*shift)/shift||0}}),define("taoClientDiagnostic/tools/stats",["lodash","taoClientDiagnostic/tools/fixedDecimals"],function(_,fixedDecimals){"use strict";return function(list,fieldName,decimals){var variance,avg,med,middle,getValue,results,sum=0,sum2=0,min=Number.MAX_VALUE,max=0,count=0,values=[];return getValue=_.isFunction(fieldName)?fieldName:function(item){
return item&&item[fieldName]||0},_.forEach(list,function(item){var value;void 0!==item&&(value=getValue(item),values.push(value),min=Math.min(min,value),max=Math.max(max,value),sum+=value,count++)}),avg=sum/(count||1),_.forEach(values,function(value){var diff=value-avg;sum2+=diff*diff}),variance=count>1?Math.sqrt(sum2/(count-1)):0,values.sort(),middle=count/2,med=(values[Math.floor(middle)]+values[Math.ceil(middle)])/2,results={min:min,max:max,sum:sum,count:count,average:avg,median:med,variance:variance},decimals&&_.forEach(results,function(value,key){results[key]=fixedDecimals(value,decimals)}),results.values=list,results}}),
define("taoQtiItem/qtiItem/core/qtiClasses",[],function(){return{_container:"taoQtiItem/qtiItem/core/Container",assessmentItem:"taoQtiItem/qtiItem/core/Item",responseProcessing:"taoQtiItem/qtiItem/core/ResponseProcessing",_simpleFeedbackRule:"taoQtiItem/qtiItem/core/response/SimpleFeedbackRule",stylesheet:"taoQtiItem/qtiItem/core/Stylesheet",math:"taoQtiItem/qtiItem/core/Math",img:"taoQtiItem/qtiItem/core/Img",object:"taoQtiItem/qtiItem/core/Object",outcomeDeclaration:"taoQtiItem/qtiItem/core/variables/OutcomeDeclaration",responseDeclaration:"taoQtiItem/qtiItem/core/variables/ResponseDeclaration",rubricBlock:"taoQtiItem/qtiItem/core/RubricBlock",
...
arg&&(arg.isRenderer?options.renderer=arg:arg instanceof $&&arg.length?options.placeholder=arg:_.isString(arg)?options.subclass=arg:_.isPlainObject(arg)?options.data=arg:console.log("invalid arg",arg,args))}),options},rendererConfigHelper}),define("taoQtiItem/qtiItem/core/Element",["jquery","lodash","class","taoQtiItem/qtiItem/helper/util","taoQtiItem/qtiItem/helper/rendererConfig"],function($,_,Class,util,rendererConfig){"use strict";var _instances={},Element=Class.extend({qtiClass:"",serial:"",relatedItem:null,init:function(serial,attributes){if(this.attributes={},this.metaData={},"object"==typeof serial&&(attributes=serial,serial=""),
serial||(serial=util.buildSerial(this.qtiClass+"_")),serial&&("string"!=typeof serial||!serial.match(/^[a-z_0-9]*$/i)))throw"invalid QTI serial : ("+typeof serial+") "+serial;if(_instances[serial])throw"a QTI Element with the same serial already exists "+serial;_instances[serial]=this,this.serial=serial,this.setAttributes(attributes||{}),"function"==typeof this.initContainer&&this.initContainer(arguments[2]||""),"function"==typeof this.initObject&&this.initObject()},is:function(qtiClass){return qtiClass===this.qtiClass},placeholder:function(){return"{{"+this.serial+"}}"},getSerial:function(){return this.serial},getUsedIdentifiers:function(){
var usedIds={},elts=this.getComposingElements();for(var i in elts){var elt=elts[i],id=elt.attr("identifier");id&&(usedIds[id]=elt)}return usedIds},getUsedIds:function(){var usedIds=[];return _.forEach(this.getComposingElements(),function(elt){var id=elt.attr("id");id&&!_.contains(usedIds,id)&&usedIds.push(id)}),usedIds},attr:function(name,value){if(name)if(void 0!==value)this.attributes[name]=value;else if("object"==typeof name)for(var prop in name)this.attr(prop,name[prop]);else if("string"==typeof name)return void 0===this.attributes[name]?void 0:this.attributes[name];return this},data:function(name,value){
if(name)if(void 0!==value)this.metaData[name]=value,$(document).trigger("metaChange.qti-widget",{element:this,key:name,value:value});else if("object"==typeof name)for(var prop in name)this.data(prop,name[prop]);else if("string"==typeof name)return void 0===this.metaData[name]?void 0:this.metaData[name];return this},removeData:function(name){return delete this.metaData[name],this},removeAttr:function(name){return this.removeAttributes(name)},setAttributes:function(attributes){return this.attributes=attributes,this},getAttributes:function(){return _.clone(this.attributes)},removeAttributes:function(attrNames){"string"==typeof attrNames&&(attrNames=[attrNames])
;for(var i in attrNames)delete this.attributes[attrNames[i]];return this},getComposingElements:function(){function append(element){elts[element.getSerial()]=element,elts=_.extend(elts,element.getComposingElements())}var elts={};return"function"==typeof this.initContainer&&append(this.getBody()),"function"==typeof this.initObject&&append(this.getObject()),_.each(this.metaData,function(v){Element.isA(v,"_container")?append(v):_.isArray(v)&&_.each(v,function(v){Element.isA(v,"_container")&&append(v)})}),elts},getUsedClasses:function(){var ret=[this.qtiClass],composingElts=this.getComposingElements();return _.each(composingElts,function(elt){
ret.push(elt.qtiClass)}),_.uniq(ret)},find:function(serial){var found=null;if("function"==typeof this.initObject){var object=this.getObject();object.serial===serial&&(found={parent:this,element:object,location:"object"})}return found||"function"!=typeof this.initContainer||(found=this.getBody().find(serial,this)),found},parent:function(){var item=this.getRelatedItem();if(item){var found=item.find(this.getSerial());if(found)return found.parent}return null},setRelatedItem:function(item,recursive){if(recursive=void 0===recursive||recursive,Element.isA(item,"assessmentItem")){this.relatedItem=item;var composingElts=this.getComposingElements()
;for(var i in composingElts)composingElts[i].setRelatedItem(item,!1)}},getRelatedItem:function(){var ret=null;return Element.isA(this.relatedItem,"assessmentItem")&&(ret=this.relatedItem),ret},setRenderer:function(renderer){if(!renderer||!renderer.isRenderer)throw"invalid qti rendering engine";this.renderer=renderer;var elts=this.getComposingElements();for(var serial in elts)elts[serial].setRenderer(renderer)},getRenderer:function(){return this.renderer},render:function(){var args=rendererConfig.getOptionsFromArguments(arguments),_renderer=args.renderer||this.getRenderer(),tplData={},defaultData={tag:this.qtiClass,serial:this.serial,
attributes:this.getAttributes()};if(!_renderer)throw"render: no renderer found for the element "+this.qtiClass+":"+this.serial;"function"==typeof this.initContainer&&(defaultData.body=this.getBody().render(args.renderer)),"function"==typeof this.initObject&&(defaultData.object={attributes:this.object.getAttributes()},defaultData.object.attributes.data=_renderer.resolveUrl(this.object.attr("data"))),tplData=_.merge(defaultData,args.data||{}),tplData=_renderer.getData(this,tplData,args.subclass);var rendering=_renderer.renderTpl(this,tplData,args.subclass);return args.placeholder&&args.placeholder.replaceWith(rendering),rendering},
postRender:function(data,altClassName,renderer){var postRenderers=[],_renderer=renderer||this.getRenderer();if("function"==typeof this.initContainer&&(postRenderers=this.getBody().postRender(data,"",renderer)),!_renderer)throw"postRender: no renderer found for the element "+this.qtiClass+":"+this.serial;return postRenderers.push(_renderer.postRender(this,data,altClassName)),_.compact(postRenderers)},getContainer:function($scope,subclass){var renderer=this.getRenderer();if(renderer)return renderer.getContainer(this,$scope,subclass);throw"getContainer: no renderer found for the element "+this.qtiClass+":"+this.serial},toArray:function(){var arr={
serial:this.serial,type:this.qtiClass,attributes:this.getAttributes()};return"function"==typeof this.initContainer&&(arr.body=this.getBody().toArray()),"function"==typeof this.initObject&&(arr.object=this.object.toArray()),arr},isEmpty:function(){return!1},addClass:function(className){var clazz=this.attr("class")||"";_containClass(clazz,className)||this.attr("class",clazz+(clazz.length?" ":"")+className)},hasClass:function(className){return _containClass(this.attr("class"),className)},removeClass:function(className){var clazz=this.attr("class")||"";if(clazz){var regex=new RegExp("(?:^|\\s)"+className+"(?:\\s|$)");clazz=clazz.replace(regex," ").trim(),
...
Element.isA(element,"interaction")?this.loadInteractionData(element,data):Element.isA(element,"choice")?this.loadChoiceData(element,data):Element.isA(element,"math")?this.loadMathData(element,data):Element.isA(element,"infoControl")&&this.loadPicData(element,data),element},loadInteractionData:function(interaction,data){Element.isA(interaction,"blockInteraction")&&data.prompt&&this.loadContainer(interaction.prompt.getBody(),data.prompt),this.buildInteractionChoices(interaction,data),Element.isA(interaction,"customInteraction")&&this.loadPciData(interaction,data)},buildInteractionChoices:function(interaction,data){if(data.choices){
if(Element.isA(interaction,"matchInteraction"))for(var set=0;set<2;set++){if(!data.choices[set])throw"missing match set #"+set;var matchSet=data.choices[set];for(var serial in matchSet){var choice=this.buildElement(matchSet[serial]);choice&&interaction.addChoice(choice,set)}}else for(var serial in data.choices){var choice=this.buildElement(data.choices[serial]);choice&&interaction.addChoice(choice)}if(Element.isA(interaction,"graphicGapMatchInteraction")&&data.gapImgs)for(var serial in data.gapImgs){var gapImg=this.buildElement(data.gapImgs[serial]);gapImg&&interaction.addGapImg(gapImg)}}},loadChoiceData:function(choice,data){
Element.isA(choice,"textVariableChoice")?choice.val(data.text):Element.isA(choice,"gapImg")||(Element.isA(choice,"gapText")?choice.body()||choice.body(data.text):Element.isA(choice,"containerChoice"))},loadObjectData:function(object,data){object.setAttributes(data.attributes),data._alt&&("object"===data._alt.qtiClass?object._alt=Loader.buildElement(data._alt):object._alt=data._alt)},loadMathData:function(math,data){math.ns=data.ns||{},math.setMathML(data.mathML||""),_.forIn(data.annotations||{},function(value,encoding){math.setAnnotation(encoding,value)})},loadPciData:function(pci,data){loadPortableCustomElementData(pci,data)},loadPicData:function(pic,data){
loadPortableCustomElementData(pic,data)}});return Loader}),define("taoQtiItem/qtiItem/helper/interactionHelper",["lodash","taoQtiItem/qtiItem/core/Element"],function(_,Element){return{convertChoices:function(choices,outputType){var ret=[],_this=this;return _.each(choices,function(c){if(Element.isA(c,"choice"))switch(outputType){case"serial":ret.push(c.getSerial());break;case"identifier":ret.push(c.id());break;default:ret.push(c)}else _.isArray(c)&&ret.push(_this.convertChoices(c,outputType))}),ret},findChoices:function(interaction,choices,inputType){var ret=[],_this=this;return _.each(choices,function(c){var choice
...
build:function(renderersLocations,name,defaultOptions){var NewRenderer=function(){var options=_.isPlainObject(arguments[0])?arguments[0]:{};Renderer.apply(this),this.register(renderersLocations),this.name=name||"",this.setOptions(_.defaults(options,defaultOptions||{}))};return NewRenderer.prototype=Renderer.prototype,NewRenderer}}}),define("taoItems/assets/manager",["lodash","util/url"],function(_,urlUtil){"use strict";return function(strategies,data,options){var cache={};strategies=_.isArray(strategies)?strategies:[strategies],data=data||{},options=_.defaults(options||{},{parseUrl:!0});var assetManager={_strategies:[],addStrategy:function(strategy){
if(!_.isPlainObject(strategy)||!_.isFunction(strategy.handle)||!_.isString(strategy.name))throw new TypeError("An asset resolution strategy is an object with a handle method and a name");this._strategies.push(strategy)},setStrategies:function(newStrategies){var self=this;this._strategies=[],_.forEach(newStrategies,function(strategy){_.isPlainObject(strategy)?assetManager.addStrategy(strategy):_.isFunction(strategy)&&self.addStrategy({name:"strategy_"+(self._strategies.length+1),handle:strategy})})},setData:function(key,value){_.isString(key)&&void 0!==value?data[key]=value:_.isPlainObject(key)&&(data=key)},getData:function(key){
return _.isString(key)?data[key]:data},resolve:function(url){var resolved,inputUrl;return options.cache&&cache.hasOwnProperty(url)?cache[url]:(inputUrl=options.parseUrl?urlUtil.parse(url):url,_.forEach(this._strategies,function(strategy){var result=strategy.handle(inputUrl,data);if(result)return resolved=result,!1}),resolved=resolved||"",options.cache&&(cache[url]=resolved),resolved)},resolveBy:function(name,url){var inputUrl,resolved="",strategy=_.find(this._strategies,{name:name});return strategy&&(inputUrl=options.parseUrl?urlUtil.parse(url):url,resolved=strategy.handle(inputUrl,data)),resolved},clearCache:function(){options.cache&&(cache={})}}
;return assetManager.setStrategies(strategies),assetManager}}),define("taoItems/assets/strategies",["util/url"],function(urlUtil){"use strict";var prependToUrl=function(url,base,slashcat){return!0===slashcat?base.replace(/\/$/,"")+"/"+url.directory.replace(/^\.\//,"").replace(/^\//,"")+encodeURIComponent(url.file.replace(/^\.\//,"").replace(/^\//,"")):base+url.directory.replace(/^\.?\//,"")+encodeURIComponent(url.file.replace(/^\.?\//,""))};return{baseUrl:{name:"baseUrl",handle:function(url,data){if("string"==typeof data.baseUrl&&urlUtil.isRelative(url))return prependToUrl(url,data.baseUrl,data.slashcat)}},itemCssNoCache:{name:"itemCssNoCache",
...
pciConfig.data?modules=_.defaults(modules,pciConfig.data.paths||{}):pciConfig.file&&reqConfigs.push("json!"+baseUrl+"/"+pciConfig.file)}),require(reqConfigs,function(){var runtimeModules={};"IMSPCI"===manifest.model?(modules=_.reduce(arguments,function(acc,conf){return _.defaults(acc,conf.paths||{})},modules),_.forEach(manifest.runtime.modules||{},function(paths,id){paths&&(_.isString(paths)||_.isArray(paths)&&paths.length)&&(runtimeModules[id]=paths)}),modules=_.merge(modules,runtimeModules),_.forEach(modules,function(paths,id){paths=_.isArray(paths)?paths:[paths],requireConfigAliases[id]=_.map(paths,function(path){
return baseUrl+"/"+path.replace(/\.js$/,"")})})):requireConfigAliases[manifest.typeIdentifier]=baseUrl,resolve(requireConfigAliases)},reject)})};return function(methods){var _loaded=!1,__providers={};return eventifier(_.defaults(methods||{},{_registry:{},get:function(typeIdentifier,version){if(this._registry[typeIdentifier])return version?_.find(this._registry[typeIdentifier],version):_.last(this._registry[typeIdentifier])},registerProvider:function(moduleName){return __providers[moduleName]=null,this},resetProviders:function(){return __providers={},this},loadProviders:function(){var self=this;return new Promise(function(resolve,reject){
...
parents:["flow"],abstract:!0},blockStatic:{parents:["block"],abstract:!0},inlineStatic:{parents:["inline"],abstract:!0},flow:{parents:["objectFlow"],abstract:!0},tableCell:{parents:["bodyElement"],contains:["flow"],abstract:!0},caption:{parents:["bodyElement"],contains:["inline"],xhtml:!0},col:{parents:["bodyElement"],xhtml:!0},colgroup:{parents:["bodyElement"],contains:["col"],xhtml:!0},div:{parents:["blockStatic","bodyElement","flowStatic"],contains:["flow"],xhtml:!0},dl:{parents:["blockStatic","bodyElement","flowStatic"],contains:["dlElement"],xhtml:!0},dt:{parents:["dlElement"],contains:["inline"],xhtml:!0},dd:{parents:["dlElement"],contains:["flow"],
xhtml:!0},hr:{parents:["blockStatic","bodyElement","flowStatic"],xhtml:!0},math:{parents:["blockStatic","flowStatic","inlineStatic"],xhtml:!0},li:{parents:["bodyElement"],contains:["flow"],xhtml:!0},ol:{parents:["blockStatic","bodyElement","flowStatic"],contains:["li"],xhtml:!0},ul:{parents:["blockStatic","bodyElement","flowStatic"],contains:["li"],xhtml:!0},object:{parents:["bodyElement","flowStatic","inlineStatic"],contains:["objectFlow"],xhtml:!0},param:{parents:["objectFlow"],xhtml:!0},table:{parents:["blockStatic","bodyElement","flowStatic"],contains:["caption","col","colgroup","thead","tfoot","tbody"],xhtml:!0},tbody:{parents:["bodyElement"],
contains:["tr"],xhtml:!0},tfoot:{parents:["bodyElement"],contains:["tr"],xhtml:!0},thead:{parents:["bodyElement"],contains:["tr"],xhtml:!0},td:{parents:["tableCell"],xhtml:!0},th:{parents:["tableCell"],xhtml:!0},tr:{parents:["bodyElement"],contains:["tableCell"],xhtml:!0},a:{parents:["simpleInline"],xhtml:!0},abbr:{parents:["simpleInline"],xhtml:!0},acronym:{parents:["simpleInline"],xhtml:!0},b:{parents:["simpleInline"],xhtml:!0},big:{parents:["simpleInline"],xhtml:!0},cite:{parents:["simpleInline"],xhtml:!0},code:{parents:["simpleInline"],xhtml:!0},dfn:{parents:["simpleInline"],xhtml:!0},em:{parents:["simpleInline"],xhtml:!0},i:{parents:["simpleInline"],
xhtml:!0},kbd:{parents:["simpleInline"],xhtml:!0},q:{parents:["simpleInline"],xhtml:!0},samp:{parents:["simpleInline"],xhtml:!0},small:{parents:["simpleInline"],xhtml:!0},span:{parents:["simpleInline"],xhtml:!0},strong:{parents:["simpleInline"],xhtml:!0},sub:{parents:["simpleInline"],xhtml:!0},sup:{parents:["simpleInline"],xhtml:!0},tt:{parents:["simpleInline"],xhtml:!0},var:{parents:["simpleInline"],xhtml:!0},blockquote:{parents:["simpleBlock"],xhtml:!0},address:{parents:["atomicBlock"],xhtml:!0},h1:{parents:["atomicBlock"],xhtml:!0},h2:{parents:["atomicBlock"],xhtml:!0},h3:{parents:["atomicBlock"],xhtml:!0},h4:{parents:["atomicBlock"],xhtml:!0},h5:{
...
;return Renderer.build(config.locations,config.name,config.options)}),define("taoClientDiagnostic/tools/getconfig",["lodash"],function(_){"use strict";return function(config,defaults){return _(config||{}).omit(function(value){return null===value||void 0===value}).defaults(defaults||{}).value()}}),
define("taoClientDiagnostic/tools/performances/tester",["jquery","lodash","i18n","async","context","helpers","taoClientDiagnostic/tools/stats","taoQtiItem/qtiItem/core/Loader","taoQtiItem/qtiCommonRenderer/renderers/Renderer","taoClientDiagnostic/tools/getconfig","taoClientDiagnostic/tools/diagnostic/status"],function($,_,__,async,context,helpers,stats,Loader,Renderer,getConfig,statusFactory){"use strict";function loadItem(data,done){
...
end=window.performance.now(),duration=(end-start)/_second,result={id:data.id,url:data.url,duration:duration},done(null,result)},this.getLoadedClasses())})})}var _second=1e3,_defaultTimeout=30*_second,_defaultSamples=["taoClientDiagnostic/tools/performances/data/sample1/","taoClientDiagnostic/tools/performances/data/sample2/","taoClientDiagnostic/tools/performances/data/sample3/"],_defaults={optimal:.025,threshold:.25};return function(config,diagnosticTool){var initConfig=getConfig(config,_defaults),idx=0,_samples=_.map(!_.isEmpty(initConfig.samples)&&initConfig.samples||_defaultSamples,function(sample){return idx++,{id:"sample"+idx,url:sample,
timeout:1e3*initConfig.timeout||_defaultTimeout,nb:initConfig.occurrences||10}}),optimal=initConfig.optimal,range=Math.abs(optimal-initConfig.threshold);return _samples[0].nb++,{start:function(done){var tests=[];diagnosticTool.changeStatus(__("Checking the performances...")),_.forEach(_samples,function(data){for(var cb=_.partial(loadItem,data),iterations=data.nb||1;iterations--;)tests.push(cb)}),async.series(tests,function(err,measures){var results,cursor,status,summary;if(err&&!measures.length)throw err;measures.shift(),results=stats(measures,"duration",2),cursor=range-results.average+optimal,
status=statusFactory().getStatus(cursor/range*100,"performances"),summary={performancesMin:{message:__("Minimum rendering time"),value:results.min+" s"},performancesMax:{message:__("Maximum rendering time"),value:results.max+" s"},performancesAverage:{message:__("Average rendering time"),value:results.average+" s"}},status.title=__("Workstation performances"),status.id="performances",diagnosticTool.addCustomFeedbackMsg(status,diagnosticTool.getCustomMsg("diagPerformancesCheckResult")),done(status,summary,results)})}}}}),
define("taoClientDiagnostic/tools/bandwidth/tester",["lodash","i18n","async","context","taoClientDiagnostic/tools/getconfig","taoClientDiagnostic/tools/stats","taoClientDiagnostic/tools/fixedDecimals","taoClientDiagnostic/tools/diagnostic/status"],function(_,__,async,context,getConfig,stats,fixedDecimals,statusFactory){"use strict";var _defaults={unit:.16,ideal:45,max:100},_downloadData={"10KB":{id:"10KB",file:"data/bin10KB.data",size:10240,timeout:1e3,threshold:0,nb:10},"100KB":{id:"100KB",file:"data/bin100KB.data",size:102400,timeout:2e3,threshold:0,nb:5},"1MB":{id:"1MB",file:"data/bin1MB.data",size:1048576,timeout:2e4,threshold:0,nb:3}
},download=function(data,cb){var start,end,timeoutId,url,request,self=this;if(data.threshold&&this.bandwidth<data.threshold)return cb("threshold");url=context.base_www+"js/tools/bandwidth/"+data.file+"?"+Date.now(),request=new XMLHttpRequest,request.open("GET",url,!0),request.setRequestHeader("Accept","application/octet-stream"),request.onload=function(){var duration,bytes,seconds,speed;return end=window.performance.now(),clearTimeout(timeoutId),duration=end-start,bytes=data.size,seconds=duration/1e3,speed=8*bytes/seconds/1048576,self.bandwidth=Math.max(self.bandwidth,speed),cb(null,{id:data.id,file:data.file,size:data.size,duration:duration,speed:speed})},
request.onerror=function(err){clearTimeout(timeoutId),cb(err)},timeoutId=_.delay(cb,data.timeout,"timeout"),start=window.performance.now(),request.send()};return function(config,diagnosticTool){var initConfig=getConfig(config||{},_defaults);return{start:function(done){var self=this,tests=[];diagnosticTool.changeStatus(__("Checking the bandwidth...")),_.forEach(_downloadData,function(data){for(var cb=_.bind(download,self,data),iterations=data.nb||1;iterations--;)tests.push(cb)}),this.bandwidth=0,async.series(tests,function(err,measures){
var getValue,results,summary,status,nb,duration=0,size=0,bandwidthUnit=initConfig.unit,threshold=initConfig.ideal,maxTestTakers=initConfig.max,max=threshold*bandwidthUnit;if(err&&!measures.length)throw err;getValue=function(value){var speed=0;return value&&(duration+=value.duration,size+=value.size,speed=value.speed,value.speed=fixedDecimals(speed,2)),speed},results=stats(measures,getValue,2),results.duration=fixedDecimals(duration/1e3,2),results.size=size,summary={bandwidthMin:{message:__("Minimum bandwidth"),value:results.min+" Mbps"},bandwidthMax:{message:__("Maximum bandwidth"),value:results.max+" Mbps"},bandwidthAverage:{message:__("Average bandwidth"),
value:results.average+" Mbps"}},status=statusFactory().getStatus(results.max/max*100,"bandwidth"),nb=Math.floor(results.max/bandwidthUnit),nb>maxTestTakers&&(nb=">"+maxTestTakers),status.id="bandwidth",status.title=__("Bandwidth"),status.feedback.legend=__("Number of simultaneous test takers the connection can handle"),diagnosticTool.addCustomFeedbackMsg(status,diagnosticTool.getCustomMsg("diagBandwithCheckResult")),status.quality.label=nb,nb.toString().length>2&&(status.quality.wide=!0),done(status,summary,results)})}}}}),
define("taoClientDiagnostic/tools/upload/tester",["jquery","lodash","i18n","async","util/url","taoClientDiagnostic/tools/diagnostic/status"],function($,_,__,async,urlHelper,statusFactory){"use strict";var data=[],generateStr=function(length){for(var text="",possible="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789",i=0;i<length;i++)text+=possible.charAt(Math.floor(Math.random()*possible.length));return text},upload=function(size){var url=urlHelper.route("upload","CompatibilityChecker","taoClientDiagnostic",{cache:Date.now()}),str=generateStr(size);return data=[],$.ajax({url:url,type:"POST",data:{upload:str},xhr:function(){
var xhr=new window.XMLHttpRequest,startTime=Date.now();return xhr.upload.addEventListener("progress",function(evt){if(evt.lengthComputable){var passedTime=Date.now()-startTime;data.push({time:passedTime,loaded:evt.loaded,speed:8*evt.loaded/1048576/(passedTime/1e3)})}},!1),xhr}})};return function(config,diagnosticTool){return{start:function(done){var jqXHR=upload(parseInt(config.size,10));diagnosticTool.changeStatus(__("Checking upload speed...")),jqXHR.then(function(){var avgSpeed,totalSpeed=0,maxSpeed=0,optimal=config.optimal/1024/1024;_.forEach(data,function(val){totalSpeed+=val.speed,maxSpeed<val.speed&&(maxSpeed=Math.round(100*val.speed)/100)}),
avgSpeed=Math.round(totalSpeed/data.length*100)/100;var status=statusFactory().getStatus(100/optimal*avgSpeed,"upload"),summary={uploadAvg:{message:__("Average upload speed"),value:avgSpeed+" Mbps"},uploadMax:{message:__("Max upload speed"),value:maxSpeed+" Mbps"}},result={max:maxSpeed,avg:avgSpeed,type:"upload"};status.id="upload",status.title=__("Upload speed"),diagnosticTool.addCustomFeedbackMsg(status,diagnosticTool.getCustomMsg("diagUploadCheckResult")),done(status,summary,result)})}}}}),define("taoClientDiagnostic/tools/getPlatformInfo",["lodash","jquery","util/url"],function(_,$,url){"use strict"
;function getTesterUrl(window,action,controller,extension){var document=window.document,navigator=window.navigator,screen=window.screen,params={},e=0,f=0;return params.ua=navigator.userAgent,e|=window.ActiveXObject?1:0,e|=window.opera?2:0,e|=window.chrome?4:0,e|="getBoxObjectFor"in document||"mozInnerScreenX"in window?8:0,e|="WebKitCSSMatrix"in window||"WebKitPoint"in window||"webkitStorageInfo"in window||"webkitURL"in window?16:0,e|=16&e&&-1==={}.toString.toString().indexOf("\n")?32:0,params.e=e,f|="sandbox"in document.createElement("iframe")?1:0,f|="WebSocket"in window?2:0,f|=window.Worker?4:0,f|=window.applicationCache?8:0,
f|=window.history&&window.history.pushState?16:0,f|=document.documentElement.webkitRequestFullScreen?32:0,f|="FileReader"in window?64:0,params.f=f,params.r=Math.random().toString(36).substring(7),params.w=screen.width,params.h=screen.height,url.route(action,controller,extension,params)}var defaultConfig={browserVersionAction:"whichBrowser",browserVersionController:"CompatibilityChecker",browserVersionExtension:"taoClientDiagnostic"};return function(window,config){var testerUrl;return config=_.defaults(config||{},defaultConfig),testerUrl=getTesterUrl(window,config.browserVersionAction,config.browserVersionController,config.browserVersionExtension),
new Promise(function(resolve,reject){$.ajax({url:testerUrl,success:function(platformInfo){resolve(platformInfo)},error:reject})})}}),define("taoClientDiagnostic/tools/browser/tester",["jquery","i18n","util/url","taoClientDiagnostic/tools/getconfig","taoClientDiagnostic/tools/getPlatformInfo","taoClientDiagnostic/tools/diagnostic/status"],function($,__,url,getConfig,getPlatformInfo,statusFactory){"use strict";function browserTester(config,diagnosticTool){var initConfig=getConfig(config||{},_defaults);return{start:function(done){diagnosticTool.changeStatus(__("Checking the browser...")),getPlatformInfo(window,initConfig).then(function(platformInfo){
$.post(url.route(initConfig.action,initConfig.controller,initConfig.extension),platformInfo,function(data){var percentage="success"===data.type?100:"warning"===data.type?33:0,status=statusFactory().getStatus(percentage,data),currentBrowser=platformInfo.browser+" "+platformInfo.browserVersion,currentOs=platformInfo.os+" "+platformInfo.osVersion,summary={browser:{message:__("Web browser"),value:currentBrowser},os:{message:__("Operating system"),value:currentOs}},customMsg=diagnosticTool.getCustomMsg("diagBrowserOsCheckResult")||"";status.id="browser",status.title=__("Operating system and web browser"),
customMsg=customMsg.replace(_placeHolders.CURRENT_BROWSER,currentBrowser).replace(_placeHolders.CURRENT_OS,currentOs),diagnosticTool.addCustomFeedbackMsg(status,customMsg),done(status,summary,platformInfo)},"json")})}}}var _defaults={browserVersionAction:"whichBrowser",browserVersionController:"CompatibilityChecker",browserVersionExtension:"taoClientDiagnostic",action:"check",controller:"DiagnosticChecker"},_placeHolders={CURRENT_BROWSER:"%CURRENT_BROWSER%",CURRENT_OS:"%CURRENT_OS%"};return browserTester}),define("tpl!taoClientDiagnostic/tools/diagnostic/tpl/main",["handlebars"],function(hb){
...
buffer+=escapeExpression(stack1)+'</button>\n    </div>\n\n    <ul class="plain results"></ul>\n\n    <div class="status">\n        <h2></h2>\n    </div>\n\n</div>\n'})}),define("tpl!taoClientDiagnostic/tools/diagnostic/tpl/result",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){function program1(depth0,data){var stack1,helper,buffer="";return buffer+='\n        <div class="small feedback feedback-',(helper=helpers.type)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.type,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),
buffer+=escapeExpression(stack1)+'">\n            <span class="icon icon-',(helper=helpers.type)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.type,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'"></span>\n            <span class="msg">',(helper=helpers.message)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.message,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+"</span>\n            ",stack1=helpers.if.call(depth0,depth0&&depth0.legend,{hash:{},inverse:self.noop,
fn:self.program(2,program2,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+="\n            ",stack1=helpers.if.call(depth0,depth0&&depth0.customMsg,{hash:{},inverse:self.noop,fn:self.program(4,program4,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+="\n        </div>\n        "}function program2(depth0,data){var stack1,helper,buffer="";return buffer+='<div class="legend">',(helper=helpers.legend)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.legend,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+"</div>"}
function program4(depth0,data){var stack1,helper,buffer="";return buffer+='<div class="customMsg">',(helper=helpers.customMsg)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.customMsg,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+"</div>"}function program6(depth0,data){var stack1,buffer="";return buffer+='\n        <div class="quality-bar">\n            <div class="quality-indicator',stack1=helpers.if.call(depth0,depth0&&depth0.wide,{hash:{},inverse:self.noop,fn:self.program(7,program7,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+='"',
stack1=helpers.if.call(depth0,depth0&&depth0.label,{hash:{},inverse:self.noop,fn:self.program(9,program9,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+="></div>\n        </div>\n        "}function program7(depth0,data){return" wide"}function program9(depth0,data){var stack1,helper,buffer="";return buffer+=' title="',(helper=helpers.label)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.label,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'"'}function program11(depth0,data){var stack1,helper,options,buffer=""
;return buffer+='\n    <div class="clearfix">\n        <button data-action="show-details" class="rgt btn-info small">'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},helper?helper.call(depth0,"Show Details",options):helperMissing.call(depth0,"__","Show Details",options)))+'</button>\n        <button data-action="hide-details" class="rgt btn-info small hidden">'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},
helper?helper.call(depth0,"Hide Details",options):helperMissing.call(depth0,"__","Hide Details",options)))+'</button>\n        <div class="details hidden">\n            <h2>'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},helper?helper.call(depth0,"Details",options):helperMissing.call(depth0,"__","Details",options)))+'</h2>\n            <div>\n                <table class="matrix">\n                    <tbody>\n                    ',stack1=helpers.each.call(depth0,depth0,{hash:{},inverse:self.noop,fn:self.program(12,program12,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),
...
header:__("This tool will run a number of tests in order to establish how well your current environment is suitable to run the TAO platform."),info:__("Be aware that these tests will take up to several minutes."),button:__("Begin diagnostics"),actionStore:"storeData",controller:"DiagnosticChecker",extension:"taoClientDiagnostic",actionDropId:"deleteId",storeAllRuns:!1,configurableText:{}},diagnostic={changeStatus:function(status){return this.is("rendered")&&this.controls.$status.html(status),this},store:function(type,details,done){var config=this.config;details=_.omit(details,"values"),details.type=type,
$.post(urlUtil.route(config.actionStore,config.controller,config.extension,config.storeParams),details,done,"json")},getCustomMsg:function(key){return this.config.configurableText[key]},addCustomFeedbackMsg:function(status,msg){this.hasFailed(status)&&msg&&(status.feedback.customMsg=msg)},hasFailed:function(result){return!(result&&result.feedback&&"success"===result.feedback.type)},addResult:function(result){var $result,$indicator;return this.is("rendered")&&(result.quality&&result.quality.label&&result.quality.label.toString().length>2&&(result.quality.wide=!0),$result=$(resultTpl(result)),$indicator=$result.find(".quality-indicator"),
this.controls.$results.append($result),$result.fadeIn(function(){$indicator.length&&$indicator.animate({left:result.percentage*$result.outerWidth()/100-$indicator.outerWidth()/2})})),this},prepare:function(){return this.trigger("start"),this.changeStatus(__("Starting...")),this.setState("running",!0),this.setState("done",!1),this.controls.$results.empty(),this.controls.$start.addClass("hidden"),this},finish:function(){var config=this.config;return this.controls.$start.removeClass("hidden"),config.storeAllRuns&&this.deleteIdentifier(),this.trigger("end"),this.changeStatus(__("Done!")),this.setState("running",!1),this.setState("done",!0),this},
deleteIdentifier:function(){var url=urlUtil.route(this.config.actionDropId,this.config.controller,this.config.extension);return request(url,null,"POST")},run:function(){function doCheck(tester,testerName,cb){self.trigger("starttester",testerName),self.setState(testerName,!0),require([tester.tester],function(testerFactory){testerFactory(getConfig(tester,self.config),self).start(function(status,details,results){_.assign(information,details),scores[status.id]=status,self.trigger("endtester",testerName,status),self.setState(testerName,!1),self.store(testerName,results,function(){self.addResult(status),cb()})})})}var self=this,information={},scores={},testers=[]
;return this.is("rendered")&&(this.prepare(),_.forEach(this.config.testers,function(tester,testerName){testers.push(function(cb){doCheck(tester,testerName,cb)})}),async.series(testers,function(){var total=_.min(scores,"percentage"),status=self.status.getStatus(total.percentage,"summary");status.title=__("Total"),status.id="total",self.addCustomFeedbackMsg(status,self.config.configurableText.diagTotalCheckResult),status.details=information,self.addResult(status),self.finish()})),this}};return diagnosticFactory}),
...
population:0|b[5]}}function h(a){a&&this._set(g(a))}function i(a){var b=a.toTimeString(),c=b.match(/\([a-z ]+\)/i);c&&c[0]?(c=c[0].match(/[A-Z]/g),c=c?c.join(""):void 0):(c=b.match(/[A-Z]{3,5}/g),c=c?c[0]:void 0),"GMT"===c&&(c=void 0),this.at=+a,this.abbr=c,this.offset=a.getTimezoneOffset()}function j(a){this.zone=a,this.offsetScore=0,this.abbrScore=0}function k(a,b){for(var c,d;d=6e4*((b.at-a.at)/12e4|0);)c=new i(new Date(a.at+d)),c.offset===a.offset?a=c:b=c;return a}function l(){var a,b,c,d=(new Date).getFullYear()-2,e=new i(new Date(d,0,1)),f=[e];for(c=1;c<48;c++)b=new i(new Date(d,c,1)),b.offset!==e.offset&&(a=k(e,b),f.push(a),
f.push(new i(new Date(a.at+6e4)))),e=b;for(c=0;c<4;c++)f.push(new i(new Date(d+c,0,1))),f.push(new i(new Date(d+c,6,1)));return f}function m(a,b){return a.offsetScore!==b.offsetScore?a.offsetScore-b.offsetScore:a.abbrScore!==b.abbrScore?a.abbrScore-b.abbrScore:b.zone.population-a.zone.population}function n(a,b){var c,e;for(d(b),c=0;c<b.length;c++)e=b[c],I[e]=I[e]||{},I[e][a]=!0}function o(a){var b,c,d,e=a.length,f={},g=[];for(b=0;b<e;b++){d=I[a[b].offset]||{};for(c in d)d.hasOwnProperty(c)&&(f[c]=!0)}for(b in f)f.hasOwnProperty(b)&&g.push(H[b]);return g}function p(){try{var a=Intl.DateTimeFormat().resolvedOptions().timeZone;if(a){var b=H[r(a)];if(b)return b
;z("Moment Timezone found "+a+" from the Intl api, but did not have that data loaded.")}}catch(c){}var d,e,f,g=l(),h=g.length,i=o(g),k=[];for(e=0;e<i.length;e++){for(d=new j(t(i[e]),h),f=0;f<h;f++)d.scoreOffsetAt(g[f]);k.push(d)}return k.sort(m),k.length>0?k[0].zone.name:void 0}function q(a){return D&&!a||(D=p()),D}function r(a){return(a||"").toLowerCase().replace(/\//g,"_")}function s(a){var b,c,d,e;for("string"==typeof a&&(a=[a]),b=0;b<a.length;b++)d=a[b].split("|"),c=d[0],e=r(c),F[e]=a[b],H[e]=c,d[5]&&n(e,d[2].split(" "))}function t(a,b){a=r(a);var c,d=F[a];return d instanceof h?d:"string"==typeof d?(d=new h(d),F[a]=d,
d):G[a]&&b!==t&&(c=t(G[a],t))?(d=F[a]=new h,d._set(c),d.name=H[a],d):null}function u(){var a,b=[];for(a in H)H.hasOwnProperty(a)&&(F[a]||F[G[a]])&&H[a]&&b.push(H[a]);return b.sort()}function v(a){var b,c,d,e;for("string"==typeof a&&(a=[a]),b=0;b<a.length;b++)c=a[b].split("|"),d=r(c[0]),e=r(c[1]),G[d]=e,H[d]=c[0],G[e]=d,H[e]=c[1]}function w(a){s(a.zones),v(a.links),A.dataVersion=a.version}function x(a){return x.didShowError||(x.didShowError=!0,z("moment.tz.zoneExists('"+a+"') has been deprecated in favor of !moment.tz.zone('"+a+"')")),!!t(a)}function y(a){return!(!a._a||void 0!==a._tzm)}function z(a){
"undefined"!=typeof console&&"function"==typeof console.error&&console.error(a)}function A(b){var c=Array.prototype.slice.call(arguments,0,-1),d=arguments[arguments.length-1],e=t(d),f=a.utc.apply(null,c);return e&&!a.isMoment(b)&&y(f)&&f.add(e.parse(f),"minutes"),f.tz(d),f}function B(a){return function(){return this._z?this._z.abbr(this):a.call(this)}}if(void 0!==a.tz)return z("Moment Timezone "+a.tz.version+" was already loaded "+(a.tz.dataVersion?"with data from ":"without any data")+a.tz.dataVersion),a;var D,F={},G={},H={},I={},J=a.version.split("."),K=+J[0],L=+J[1]
;(K<2||2===K&&L<6)&&z("Moment Timezone requires Moment.js >= 2.6.0. You are using Moment.js "+a.version+". See momentjs.com"),h.prototype={_set:function(a){this.name=a.name,this.abbrs=a.abbrs,this.untils=a.untils,this.offsets=a.offsets,this.population=a.population},_index:function(a){var b,c=+a,d=this.untils;for(b=0;b<d.length;b++)if(c<d[b])return b},parse:function(a){var b,c,d,e,f=+a,g=this.offsets,h=this.untils,i=h.length-1;for(e=0;e<i;e++)if(b=g[e],c=g[e+1],d=g[e?e-1:e],b<c&&A.moveAmbiguousForward?b=c:b>d&&A.moveInvalidForward&&(b=d),f<h[e]-6e4*b)return g[e];return g[i]},abbr:function(a){return this.abbrs[this._index(a)]},offset:function(a){
return this.offsets[this._index(a)]}},j.prototype.scoreOffsetAt=function(a){this.offsetScore+=Math.abs(this.zone.offset(a.at)-a.offset),this.zone.abbr(a.at).replace(/[^A-Z]/g,"")!==a.abbr&&this.abbrScore++},A.version="0.5.10",A.dataVersion="",A._zones=F,A._links=G,A._names=H,A.add=s,A.link=v,A.load=w,A.zone=t,A.zoneExists=x,A.guess=q,A.names=u,A.Zone=h,A.unpack=g,A.unpackBase60=c,A.needsOffset=y,A.moveInvalidForward=!0,A.moveAmbiguousForward=!1;var M=a.fn;a.tz=A,a.defaultZone=null,a.updateOffset=function(b,c){var d,e=a.defaultZone;void 0===b._z&&(e&&y(b)&&!b._isUTC&&(b._d=a.utc(b._a)._d,b.utc().add(e.parse(b),"minutes")),b._z=e),b._z&&(d=b._z.offset(b),
...
;return"[object Array]"===Object.prototype.toString.call(N)?(N.push("_z"),N.push("_a")):N&&(N._z=null),w({version:"2016j",
...
}),a}),define("taoClientDiagnostic/controller/Diagnostic/index",["jquery","i18n","helpers","layout/loading-bar","util/encode","ui/feedback","ui/dialog","taoClientDiagnostic/tools/diagnostic/status","moment","ui/datatable","lib/moment-timezone.min"],function($,__,helpers,loadingBar,encode,feedback,dialog,statusFactory,moment){"use strict";function formatNumber(number,digits){var nb=void 0===digits?2:Math.max(0,parseInt(digits,10)),factor=Math.pow(10,nb)||1;return Math.round(number*factor)/factor}function transformDateToLocal(date){return date=moment.tz(date,defaultDateTimeZone).tz(moment.tz.guess()).format(defaultDateFormat)}
var defaultDateTimeZone="UTC",defaultDateFormat="Y/MM/DD HH:mm:ss";return loadingBar.start(),{start:function(){function request(url,selection,message){selection&&selection.length&&(loadingBar.start(),$.ajax({url:url,data:{id:selection},dataType:"json",type:"POST",error:function(){loadingBar.stop()}}).done(function(response){loadingBar.stop(),response&&response.success?(message&&feedback().success(message),$list.datatable("refresh")):feedback().error(__("Something went wrong ...")+"<br>"+encode.html(response.error),{encodeHtml:!1})}))}function remove(selection){request(removeUrl,selection,__("The readiness check result have been removed"))}
var $container=$(".diagnostic-index"),extension=$container.data("extension")||"taoClientDiagnostic",$list=$container.find(".list"),dataset=$container.data("set"),config=$container.data("config")||{},installedExtension=$container.data("installedextension")||!1,diagnosticUrl=helpers._url("diagnostic","Diagnostic",extension),removeUrl=helpers._url("remove","Diagnostic",extension),serviceUrl=helpers._url("diagnosticData","Diagnostic",extension),performancesConfig=config.testers.performance||{},performancesOptimal=performancesConfig.optimal,performancesRange=Math.abs(performancesOptimal-performancesConfig.threshold),diagnosticStatus=statusFactory(),tools=[],actions=[],model=[]
