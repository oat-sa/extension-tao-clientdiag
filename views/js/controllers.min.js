...
responseDeclaration:!1,responseProcessing:!1,img:"taoQtiItem/qtiCommonRenderer/renderers/Img",math:"taoQtiItem/qtiCommonRenderer/renderers/Math",object:"taoQtiItem/qtiCommonRenderer/renderers/Object",table:"taoQtiItem/qtiCommonRenderer/renderers/Table",printedVariable:"taoQtiItem/qtiCommonRenderer/renderers/PrintedVariable",rubricBlock:"taoQtiItem/qtiCommonRenderer/renderers/RubricBlock",modalFeedback:"taoQtiItem/qtiCommonRenderer/renderers/ModalFeedback",prompt:"taoQtiItem/qtiCommonRenderer/renderers/interactions/Prompt",choiceInteraction:"taoQtiItem/qtiCommonRenderer/renderers/interactions/ChoiceInteraction",
...
end=window.performance.now(),duration=(end-start)/_second,result={id:data.id,url:data.url,duration:duration},done(null,result)},this.getLoadedClasses())})})}var _second=1e3,_defaultTimeout=30*_second,_defaultSamples=["taoClientDiagnostic/tools/performances/data/sample1/","taoClientDiagnostic/tools/performances/data/sample2/","taoClientDiagnostic/tools/performances/data/sample3/"],_defaults={optimal:.025,threshold:.25},_messages=[{title:__("Workstation performances"),status:__("Checking the performances..."),performancesMin:__("Minimum rendering time"),performancesMax:__("Maximum rendering time"),performancesAverage:__("Average rendering time")}]
;return function(config,diagnosticTool){var initConfig=getConfig(config,_defaults),idx=0,_samples=_.map(!_.isEmpty(initConfig.samples)&&initConfig.samples||_defaultSamples,function(sample){return idx++,{id:"sample"+idx,url:sample,timeout:1e3*initConfig.timeout||_defaultTimeout,nb:initConfig.occurrences||10}}),optimal=initConfig.optimal,range=Math.abs(optimal-initConfig.threshold),level=Math.min(Math.max(parseInt(initConfig.level,10),1),_messages.length)-1;return _samples[0].nb++,{start:function(done){var tests=[];diagnosticTool.changeStatus(_messages[level].status),_.forEach(_samples,function(data){
for(var cb=_.partial(loadItem,data),iterations=data.nb||1;iterations--;)tests.push(cb)}),async.series(tests,function(err,measures){var results,cursor,status,summary;if(err&&!measures.length)throw err;measures.shift(),results=stats(measures,"duration",2),cursor=range-results.average+optimal,status=statusFactory().getStatus(cursor/range*100,"performances"),summary={performancesMin:{message:_messages[level].performancesMin,value:results.min+" s"},performancesMax:{message:_messages[level].performancesMax,value:results.max+" s"},performancesAverage:{message:_messages[level].performancesAverage,value:results.average+" s"}},status.title=_messages[level].title,
status.id=initConfig.id||"performances",diagnosticTool.addCustomFeedbackMsg(status,diagnosticTool.getCustomMsg("diagPerformancesCheckResult")),done(status,summary,results)})}}}}),define("taoClientDiagnostic/tools/bandwidth/tester",["lodash","i18n","async","context","taoClientDiagnostic/tools/getconfig","taoClientDiagnostic/tools/stats","taoClientDiagnostic/tools/fixedDecimals","taoClientDiagnostic/tools/diagnostic/status"],function(_,__,async,context,getConfig,stats,fixedDecimals,statusFactory){"use strict";var _defaults={unit:.16,ideal:45,max:100},_downloadData={"10KB":{id:"10KB",file:"data/bin10KB.data",size:10240,timeout:1e3,threshold:0,nb:10},"100KB":{
id:"100KB",file:"data/bin100KB.data",size:102400,timeout:2e3,threshold:0,nb:5},"1MB":{id:"1MB",file:"data/bin1MB.data",size:1048576,timeout:2e4,threshold:0,nb:3}},_messages=[{title:__("Bandwidth"),status:__("Checking the bandwidth..."),legend:__("Number of simultaneous test takers the connection can handle"),bandwidthMin:__("Minimum bandwidth"),bandwidthMax:__("Maximum bandwidth"),bandwidthAverage:__("Average bandwidth")},{title:__("Media intensive bandwidth"),status:__("Checking the media intensive bandwidth..."),legend:__("Number of simultaneous test takers the connection can handle with media intensive"),bandwidthMin:__("Minimum intensive bandwidth"),
bandwidthMax:__("Maximum intensive bandwidth"),bandwidthAverage:__("Average intensive bandwidth")}],download=function(data,cb){var start,end,timeoutId,url,request,self=this;if(data.threshold&&this.bandwidth<data.threshold)return cb("threshold");url=context.base_www+"js/tools/bandwidth/"+data.file+"?"+Date.now(),request=new XMLHttpRequest,request.open("GET",url,!0),request.setRequestHeader("Accept","application/octet-stream"),request.onload=function(){var duration,bytes,seconds,speed;return end=window.performance.now(),clearTimeout(timeoutId),duration=end-start,bytes=data.size,seconds=duration/1e3,speed=8*bytes/seconds/1048576,
self.bandwidth=Math.max(self.bandwidth,speed),cb(null,{id:data.id,file:data.file,size:data.size,duration:duration,speed:speed})},request.onerror=function(err){clearTimeout(timeoutId),cb(err)},timeoutId=_.delay(cb,data.timeout,"timeout"),start=window.performance.now(),request.send()};return function(config,diagnosticTool){var initConfig=getConfig(config,_defaults),level=Math.min(Math.max(parseInt(initConfig.level,10),1),_messages.length)-1;return{start:function(done){var self=this,tests=[];diagnosticTool.changeStatus(_messages[level].status),_.forEach(_downloadData,function(data){
for(var cb=_.bind(download,self,data),iterations=data.nb||1;iterations--;)tests.push(cb)}),this.bandwidth=0,async.series(tests,function(err,measures){var getValue,results,summary,status,nb,duration=0,size=0,bandwidthUnit=initConfig.unit,threshold=initConfig.ideal,maxTestTakers=initConfig.max,max=threshold*bandwidthUnit;if(err&&!measures.length)throw err;getValue=function(value){var speed=0;return value&&(duration+=value.duration,size+=value.size,speed=value.speed,value.speed=fixedDecimals(speed,2)),speed},results=stats(measures,getValue,2),results.duration=fixedDecimals(duration/1e3,2),results.size=size,summary={bandwidthMin:{
message:_messages[level].bandwidthMin,value:results.min+" Mbps"},bandwidthMax:{message:_messages[level].bandwidthMax,value:results.max+" Mbps"},bandwidthAverage:{message:_messages[level].bandwidthAverage,value:results.average+" Mbps"}},status=statusFactory().getStatus(results.max/max*100,"bandwidth"),nb=Math.floor(results.max/bandwidthUnit),nb>maxTestTakers&&(nb=">"+maxTestTakers),status.id=initConfig.id||"bandwidth",status.title=_messages[level].title,status.feedback.legend=_messages[level].legend,diagnosticTool.addCustomFeedbackMsg(status,diagnosticTool.getCustomMsg("diagBandwithCheckResult")),status.quality.label=nb,
nb.toString().length>2&&(status.quality.wide=!0),done(status,summary,results)})}}}}),define("taoClientDiagnostic/tools/upload/tester",["jquery","lodash","i18n","async","util/url","taoClientDiagnostic/tools/getconfig","taoClientDiagnostic/tools/diagnostic/status"],function($,_,__,async,urlHelper,getConfig,statusFactory){"use strict";var data=[],_defaults={size:1048576,optimal:1048576},_messages=[{title:__("Upload speed"),status:__("Checking upload speed..."),uploadAvg:__("Average upload speed"),uploadMax:__("Max upload speed")}],generateStr=function(length){var i,text="",possible="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789"
;for(i=0;i<length;i++)text+=possible.charAt(Math.floor(Math.random()*possible.length));return text},upload=function(size){var url=urlHelper.route("upload","CompatibilityChecker","taoClientDiagnostic",{cache:Date.now()}),str=generateStr(size);return data=[],$.ajax({url:url,type:"POST",data:{upload:str},xhr:function(){var xhr=new window.XMLHttpRequest,startTime=Date.now();return xhr.upload.addEventListener("progress",function(evt){var passedTime;evt.lengthComputable&&(passedTime=Date.now()-startTime,data.push({time:passedTime,loaded:evt.loaded,speed:8*evt.loaded/1048576/(passedTime/1e3)}))},!1),xhr}})};return function(config,diagnosticTool){
var initConfig=getConfig(config,_defaults),level=Math.min(Math.max(parseInt(initConfig.level,10),1),_messages.length)-1;return{start:function(done){var jqXHR=upload(parseInt(initConfig.size,10));diagnosticTool.changeStatus(_messages[level].status),jqXHR.then(function(){var avgSpeed,status,summary,result,totalSpeed=0,maxSpeed=0,optimal=initConfig.optimal/1048576;_.forEach(data,function(val){totalSpeed+=val.speed,maxSpeed<val.speed&&(maxSpeed=Math.round(100*val.speed)/100)}),avgSpeed=Math.round(totalSpeed/data.length*100)/100,status=statusFactory().getStatus(100/optimal*avgSpeed,"upload"),summary={uploadAvg:{message:_messages[level].uploadAvg,
value:avgSpeed+" Mbps"},uploadMax:{message:_messages[level].uploadMax,value:maxSpeed+" Mbps"}},result={max:maxSpeed,avg:avgSpeed,type:"upload"},status.id=initConfig.id||"upload",status.title=_messages[level].title,diagnosticTool.addCustomFeedbackMsg(status,diagnosticTool.getCustomMsg("diagUploadCheckResult")),done(status,summary,result)})}}}}),define("taoClientDiagnostic/tools/getPlatformInfo",["lodash","jquery","util/url"],function(_,$,url){"use strict";function getTesterUrl(window,action,controller,extension){var document=window.document,navigator=window.navigator,screen=window.screen,params={},e=0,f=0;return params.ua=navigator.userAgent,
e|=window.ActiveXObject?1:0,e|=window.opera?2:0,e|=window.chrome?4:0,e|="getBoxObjectFor"in document||"mozInnerScreenX"in window?8:0,e|="WebKitCSSMatrix"in window||"WebKitPoint"in window||"webkitStorageInfo"in window||"webkitURL"in window?16:0,e|=16&e&&-1==={}.toString.toString().indexOf("\n")?32:0,params.e=e,f|="sandbox"in document.createElement("iframe")?1:0,f|="WebSocket"in window?2:0,f|=window.Worker?4:0,f|=window.applicationCache?8:0,f|=window.history&&window.history.pushState?16:0,f|=document.documentElement.webkitRequestFullScreen?32:0,f|="FileReader"in window?64:0,params.f=f,params.r=Math.random().toString(36).substring(7),params.w=screen.width,
params.h=screen.height,url.route(action,controller,extension,params)}var defaultConfig={browserVersionAction:"whichBrowser",browserVersionController:"CompatibilityChecker",browserVersionExtension:"taoClientDiagnostic"};return function(window,config){var testerUrl;return config=_.defaults(config||{},defaultConfig),testerUrl=getTesterUrl(window,config.browserVersionAction,config.browserVersionController,config.browserVersionExtension),new Promise(function(resolve,reject){$.ajax({url:testerUrl,success:function(platformInfo){resolve(platformInfo)},error:reject})})}}),
define("taoClientDiagnostic/tools/browser/tester",["jquery","i18n","util/url","taoClientDiagnostic/tools/getconfig","taoClientDiagnostic/tools/getPlatformInfo","taoClientDiagnostic/tools/diagnostic/status"],function($,__,url,getConfig,getPlatformInfo,statusFactory){"use strict";function browserTester(config,diagnosticTool){var initConfig=getConfig(config,_defaults),level=Math.min(Math.max(parseInt(initConfig.level,10),1),_messages.length)-1;return{start:function(done){diagnosticTool.changeStatus(_messages[level].status),getPlatformInfo(window,initConfig).then(function(platformInfo){
$.post(url.route(initConfig.action,initConfig.controller,initConfig.extension),platformInfo,function(data){var percentage="success"===data.type?100:"warning"===data.type?33:0,status=statusFactory().getStatus(percentage,data),currentBrowser=platformInfo.browser+" "+platformInfo.browserVersion,currentOs=platformInfo.os+" "+platformInfo.osVersion,summary={browser:{message:_messages[level].browser,value:currentBrowser},os:{message:_messages[level].os,value:currentOs}},customMsg=diagnosticTool.getCustomMsg("diagBrowserOsCheckResult")||"";status.id=initConfig.id||"browser",status.title=_messages[level].title,
...
deleteIdentifier:function(){var url=urlUtil.route(this.config.actionDropId,this.config.controller,this.config.extension);return request(url,null,"POST")},run:function(){function doCheck(tester,testerId,cb){self.trigger("starttester",testerId),self.setState(testerId,!0),require([tester.tester],function(testerFactory){testerFactory(getConfig(tester,self.config),self).start(function(status,details,results){_.forEach(details,function(info){information.push(info)}),scores[status.id]=status,self.trigger("endtester",testerId,status),self.setState(testerId,!1),self.store(testerId,results,function(){self.addResult(status),cb()})})})}
var self=this,information=[],scores={},testers=[];return this.is("rendered")&&(this.prepare(),_.forEach(this.config.testers,function(tester,testerId){tester.id=testerId,tester.enabled&&testers.push(function(cb){doCheck(tester,testerId,cb)})}),async.series(testers,function(){var total=_.min(scores,"percentage"),status=self.status.getStatus(total.percentage,"summary");status.title=__("Total"),status.id="total",self.addCustomFeedbackMsg(status,self.config.configurableText.diagTotalCheckResult),status.details=information,self.addResult(status),self.finish()})),this}};return diagnosticFactory}),
...
loading:__("Loading")},tools:tools,actions:actions,selectable:!0,model:model},dataset)}};return loadingBar.start(),taoDiagnosticCtlr}),function(c){var d=document,a="appendChild",i="styleSheet",s=d.createElement("style");s.type="text/css",d.getElementsByTagName("head")[0][a](s),s[i]?s[i].cssText=c:s[a](d.createTextNode(c))
