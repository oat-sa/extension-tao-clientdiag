...
"function"==typeof this.initObject&&(defaultData.object={attributes:this.object.getAttributes()},defaultData.object.attributes.data=_renderer.resolveUrl(this.object.attr("data"))),tplData=_.merge(defaultData,args.data||{}),tplData=_renderer.getData(this,tplData,args.subclass);var rendering=_renderer.renderTpl(this,tplData,args.subclass);return args.placeholder&&args.placeholder.replaceWith(rendering),rendering},postRender:function(data,altClassName,renderer){var postRenderers=[],_renderer=renderer||this.getRenderer();if("function"==typeof this.initContainer&&(postRenderers=this.getBody().postRender(data,"",renderer)),!_renderer)throw"postRender: no renderer found for the element "+this.qtiClass+":"+this.serial;
...
var absUrl="",runtimeLocations=this.getOption("runtimeLocations");if(runtimeLocations&&_.size(runtimeLocations)&&_.forIn(runtimeLocations,function(runtimeLocation,typeIdentifier){if(0===relUrl.indexOf(typeIdentifier))return absUrl=relUrl.replace(typeIdentifier,runtimeLocation),!1}),absUrl)return absUrl;var baseUrl=this.getOption("baseUrl")||getDocumentBaseUrl();return baseUrl+relUrl},this.setAreaBroker=function(areaBroker){this._areaBroker=areaBroker},this.getAreaBroker=function(){if(this._areaBroker)return this._areaBroker}};return{build:function(renderersLocations,name,defaultOptions){var NewRenderer=function(){var options=_.isPlainObject(arguments[0])?arguments[0]:{};
Renderer.apply(this),this.register(renderersLocations),this.name=name||"",this.setOptions(_.defaults(options,defaultOptions||{}))};return NewRenderer.prototype=Renderer.prototype,NewRenderer}}}),define("taoItems/assets/manager",["lodash","util/url"],function(_,urlUtil){"use strict";var assetManagerFactory=function(strategies,data,options){var cache={};strategies=_.isArray(strategies)?strategies:[strategies],data=data||{},options=_.defaults(options||{},{parseUrl:!0});var assetManager={_strategies:[],addStrategy:function(strategy){if(!_.isPlainObject(strategy)||!_.isFunction(strategy.handle)||!_.isString(strategy.name))throw new TypeError("An asset resolution strategy is an object with a handle method and a name");
...
outcomeDeclaration:!1,responseDeclaration:!1,responseProcessing:!1,img:"taoQtiItem/qtiCommonRenderer/renderers/Img",math:"taoQtiItem/qtiCommonRenderer/renderers/Math",object:"taoQtiItem/qtiCommonRenderer/renderers/Object",table:"taoQtiItem/qtiCommonRenderer/renderers/Table",rubricBlock:"taoQtiItem/qtiCommonRenderer/renderers/RubricBlock",modalFeedback:"taoQtiItem/qtiCommonRenderer/renderers/ModalFeedback",prompt:"taoQtiItem/qtiCommonRenderer/renderers/interactions/Prompt",choiceInteraction:"taoQtiItem/qtiCommonRenderer/renderers/interactions/ChoiceInteraction",extendedTextInteraction:"taoQtiItem/qtiCommonRenderer/renderers/interactions/ExtendedTextInteraction",
...
$container.remove(),end=window.performance.now(),duration=(end-start)/_second,result={id:data.id,url:data.url,duration:duration},done(null,result)},this.getLoadedClasses())})})}var _second=1e3,_defaultTimeout=30*_second,_defaultOccurrencesCount=10,_defaultSamples=["taoClientDiagnostic/tools/performances/data/sample1/","taoClientDiagnostic/tools/performances/data/sample2/","taoClientDiagnostic/tools/performances/data/sample3/"],_defaults={optimal:.025,threshold:.25},_sampleBaseId="sample",performancesTester=function(config,diagnosticTool){var initConfig=getConfig(config,_defaults),idx=0,_samples=_.map(!_.isEmpty(initConfig.samples)&&initConfig.samples||_defaultSamples,function(sample){
return idx++,{id:_sampleBaseId+idx,url:sample,timeout:1e3*initConfig.timeout||_defaultTimeout,nb:initConfig.occurrences||_defaultOccurrencesCount}}),optimal=initConfig.optimal,range=Math.abs(optimal-initConfig.threshold);return _samples[0].nb++,{start:function(done){var tests=[];diagnosticTool.changeStatus(__("Checking the performances...")),_.forEach(_samples,function(data){for(var cb=_.partial(loadItem,data),iterations=data.nb||1;iterations--;)tests.push(cb)}),async.series(tests,function(err,measures){var results,cursor,status,summary,decimals=2;if(err&&!measures.length)throw err;measures.shift(),results=stats(measures,"duration",decimals),cursor=range-results.average+optimal,
status=statusFactory().getStatus(cursor/range*100,"performances"),summary={performancesMin:{message:__("Minimum rendering time"),value:results.min+" s"},performancesMax:{message:__("Maximum rendering time"),value:results.max+" s"},performancesAverage:{message:__("Average rendering time"),value:results.average+" s"}},status.title=__("Workstation performances"),status.id="performances",diagnosticTool.addCustomFeedbackMsg(status,diagnosticTool.getCustomMsg("diagPerformancesCheckResult")),done(status,summary,results)})}}};return performancesTester}),define("taoClientDiagnostic/tools/bandwidth/tester",["lodash","i18n","async","context","taoClientDiagnostic/tools/getconfig","taoClientDiagnostic/tools/stats","taoClientDiagnostic/tools/fixedDecimals","taoClientDiagnostic/tools/diagnostic/status"],function(_,__,async,context,getConfig,stats,fixedDecimals,statusFactory){
"use strict";var _kilo=1024,_mega=_kilo*_kilo,_second=1e3,_defaults={unit:.16,ideal:45,max:100},_downloadData={"10KB":{id:"10KB",file:"data/bin10KB.data",size:10*_kilo,timeout:_second,threshold:0,nb:10},"100KB":{id:"100KB",file:"data/bin100KB.data",size:100*_kilo,timeout:2*_second,threshold:0,nb:5},"1MB":{id:"1MB",file:"data/bin1MB.data",size:_mega,timeout:20*_second,threshold:0,nb:3}},download=function(data,cb){var start,end,timeoutId,url,request,self=this;return data.threshold&&this.bandwidth<data.threshold?cb("threshold"):(url=context.base_www+"js/tools/bandwidth/"+data.file+"?"+Date.now(),request=new XMLHttpRequest,request.open("GET",url,!0),request.setRequestHeader("Accept","application/octet-stream"),
request.onload=function(){var duration,bytes,seconds,speed;return end=window.performance.now(),clearTimeout(timeoutId),duration=end-start,bytes=data.size,seconds=duration/_second,speed=8*bytes/seconds/_mega,self.bandwidth=Math.max(self.bandwidth,speed),cb(null,{id:data.id,file:data.file,size:data.size,duration:duration,speed:speed})},request.onerror=function(err){clearTimeout(timeoutId),cb(err)},timeoutId=_.delay(cb,data.timeout,"timeout"),start=window.performance.now(),void request.send())},bandwidthTester=function(config,diagnosticTool){var initConfig=getConfig(config||{},_defaults);return{start:function(done){var self=this,tests=[];diagnosticTool.changeStatus(__("Checking the bandwidth...")),
_.forEach(_downloadData,function(data){for(var cb=_.bind(download,self,data),iterations=data.nb||1;iterations--;)tests.push(cb)}),this.bandwidth=0,async.series(tests,function(err,measures){var getValue,results,summary,status,nb,duration=0,size=0,decimals=2,bandwidthUnit=initConfig.unit,threshold=initConfig.ideal,maxTestTakers=initConfig.max,max=threshold*bandwidthUnit;if(err&&!measures.length)throw err;getValue=function(value){var speed=0;return value&&(duration+=value.duration,size+=value.size,speed=value.speed,value.speed=fixedDecimals(speed,decimals)),speed},results=stats(measures,getValue,decimals),results.duration=fixedDecimals(duration/_second,decimals),
results.size=size,summary={bandwidthMin:{message:__("Minimum bandwidth"),value:results.min+" Mbps"},bandwidthMax:{message:__("Maximum bandwidth"),value:results.max+" Mbps"},bandwidthAverage:{message:__("Average bandwidth"),value:results.average+" Mbps"}},status=statusFactory().getStatus(results.max/max*100,"bandwidth"),nb=Math.floor(results.max/bandwidthUnit),nb>maxTestTakers&&(nb=">"+maxTestTakers),status.id="bandwidth",status.title=__("Bandwidth"),status.feedback.legend=__("Number of simultaneous test takers the connection can handle"),diagnosticTool.addCustomFeedbackMsg(status,diagnosticTool.getCustomMsg("diagBandwithCheckResult")),status.quality.label=nb,
nb.toString().length>2&&(status.quality.wide=!0),done(status,summary,results)})}}};return bandwidthTester}),define("taoClientDiagnostic/tools/upload/tester",["jquery","lodash","i18n","async","util/url","taoClientDiagnostic/tools/diagnostic/status"],function($,_,__,async,urlHelper,statusFactory){"use strict";var _kilo=1024,_mega=_kilo*_kilo,data=[],generateStr=function(length){for(var text="",possible="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789",i=0;i<length;i++)text+=possible.charAt(Math.floor(Math.random()*possible.length));return text},upload=function(size){var url=urlHelper.route("upload","CompatibilityChecker","taoClientDiagnostic",{cache:Date.now()
}),str=generateStr(size);return data=[],$.ajax({url:url,type:"POST",data:{upload:str},xhr:function(){var xhr=new window.XMLHttpRequest,startTime=Date.now();return xhr.upload.addEventListener("progress",function(evt){if(evt.lengthComputable){var passedTime=Date.now()-startTime;data.push({time:passedTime,loaded:evt.loaded,speed:8*evt.loaded/_mega/(passedTime/1e3)})}},!1),xhr}})},uploadTester=function(config,diagnosticTool){return{start:function(done){var jqXHR=upload(parseInt(config.size,10));diagnosticTool.changeStatus(__("Checking upload speed...")),jqXHR.then(function(){var avgSpeed,totalSpeed=0,maxSpeed=0,optimal=config.optimal/1024/1024;_.forEach(data,function(val){
totalSpeed+=val.speed,maxSpeed<val.speed&&(maxSpeed=Math.round(100*val.speed)/100)}),avgSpeed=Math.round(totalSpeed/data.length*100)/100;var status=statusFactory().getStatus(100/optimal*avgSpeed,"upload"),summary={uploadAvg:{message:__("Average upload speed"),value:avgSpeed+" Mbps"},uploadMax:{message:__("Max upload speed"),value:maxSpeed+" Mbps"}},result={max:maxSpeed,avg:avgSpeed,type:"upload"};status.id="upload",status.title=__("Upload speed"),diagnosticTool.addCustomFeedbackMsg(status,diagnosticTool.getCustomMsg("diagUploadCheckResult")),done(status,summary,result)})}}};return uploadTester}),define("taoClientDiagnostic/tools/getPlatformInfo",["lodash","jquery","util/url"],function(_,$,url){
"use strict";function getTesterUrl(window,action,controller,extension){var document=window.document,navigator=window.navigator,screen=window.screen,params={},e=0,f=0;return params.ua=navigator.userAgent,e|=window.ActiveXObject?1:0,e|=window.opera?2:0,e|=window.chrome?4:0,e|="getBoxObjectFor"in document||"mozInnerScreenX"in window?8:0,e|="WebKitCSSMatrix"in window||"WebKitPoint"in window||"webkitStorageInfo"in window||"webkitURL"in window?16:0,e|=16&e&&{}.toString.toString().indexOf("\n")===-1?32:0,params.e=e,f|="sandbox"in document.createElement("iframe")?1:0,f|="WebSocket"in window?2:0,f|=window.Worker?4:0,f|=window.applicationCache?8:0,f|=window.history&&window.history.pushState?16:0,
f|=document.documentElement.webkitRequestFullScreen?32:0,f|="FileReader"in window?64:0,params.f=f,params.r=Math.random().toString(36).substring(7),params.w=screen.width,params.h=screen.height,url.route(action,controller,extension,params)}var defaultConfig={browserVersionAction:"whichBrowser",browserVersionController:"CompatibilityChecker",browserVersionExtension:"taoClientDiagnostic"};return function(window,config){var testerUrl;return config=_.defaults(config||{},defaultConfig),testerUrl=getTesterUrl(window,config.browserVersionAction,config.browserVersionController,config.browserVersionExtension),new Promise(function(resolve,reject){$.ajax({url:testerUrl,success:function(platformInfo){
resolve(platformInfo)},error:reject})})}}),define("taoClientDiagnostic/tools/browser/tester",["jquery","i18n","util/url","taoClientDiagnostic/tools/getconfig","taoClientDiagnostic/tools/getPlatformInfo","taoClientDiagnostic/tools/diagnostic/status"],function($,__,url,getConfig,getPlatformInfo,statusFactory){"use strict";function browserTester(config,diagnosticTool){var initConfig=getConfig(config||{},_defaults);return{start:function(done){diagnosticTool.changeStatus(__("Checking the browser...")),getPlatformInfo(window,initConfig).then(function(platformInfo){$.post(url.route(initConfig.action,initConfig.controller,initConfig.extension),platformInfo,function(data){
var percentage="success"===data.type?100:"warning"===data.type?33:0,status=statusFactory().getStatus(percentage,data),currentBrowser=platformInfo.browser+" "+platformInfo.browserVersion,currentOs=platformInfo.os+" "+platformInfo.osVersion,summary={browser:{message:__("Web browser"),value:currentBrowser},os:{message:__("Operating system"),value:currentOs}},customMsg=diagnosticTool.getCustomMsg("diagBrowserOsCheckResult")||"";status.id="browser",status.title=__("Operating system and web browser"),customMsg=customMsg.replace(_placeHolders.CURRENT_BROWSER,currentBrowser).replace(_placeHolders.CURRENT_OS,currentOs),diagnosticTool.addCustomFeedbackMsg(status,customMsg),
done(status,summary,platformInfo)},"json")})}}}var _defaults={browserVersionAction:"whichBrowser",browserVersionController:"CompatibilityChecker",browserVersionExtension:"taoClientDiagnostic",action:"check",controller:"DiagnosticChecker"},_placeHolders={CURRENT_BROWSER:"%CURRENT_BROWSER%",CURRENT_OS:"%CURRENT_OS%"};return browserTester}),define("tpl!taoClientDiagnostic/tools/diagnostic/tpl/main",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){function program1(depth0,data){var stack1,helper,buffer="";return buffer+="<p>",(helper=helpers.header)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.header,
stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+"</p>"}function program3(depth0,data){var stack1,helper,buffer="";return buffer+="<p>",(helper=helpers.info)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.info,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+"</p>"}this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression,self=this;return buffer+='<div class="diagnostics-main-area">\n\n    <h1>',
(helper=helpers.title)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.title,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'</h1>\n\n    <div class="intro">\n        ',stack1=helpers.if.call(depth0,depth0&&depth0.header,{hash:{},inverse:self.noop,fn:self.program(1,program1,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+="\n        ",stack1=helpers.if.call(depth0,depth0&&depth0.info,{hash:{},inverse:self.noop,fn:self.program(3,program3,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+='\n    </div>\n\n    <div class="clearfix">\n        <button data-action="test-launcher" class="btn-info small rgt">',
(helper=helpers.button)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.button,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'</button>\n    </div>\n\n    <ul class="plain results"></ul>\n\n    <div class="status">\n        <h2></h2>\n    </div>\n\n</div>\n'})}),define("tpl!taoClientDiagnostic/tools/diagnostic/tpl/result",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){function program1(depth0,data){var stack1,helper,buffer="";return buffer+='\n        <div class="small feedback feedback-',(helper=helpers.type)?stack1=helper.call(depth0,{
...
return a>96?a-87:a>64?a-29:a-48}function c(a){var c,d=0,e=a.split("."),f=e[0],g=e[1]||"",h=1,i=0,j=1;for(45===a.charCodeAt(0)&&(d=1,j=-1),d;d<f.length;d++)c=b(f.charCodeAt(d)),i=60*i+c;for(d=0;d<g.length;d++)h/=60,c=b(g.charCodeAt(d)),i+=c*h;return i*j}function d(a){for(var b=0;b<a.length;b++)a[b]=c(a[b])}function e(a,b){for(var c=0;c<b;c++)a[c]=Math.round((a[c-1]||0)+6e4*a[c]);a[b-1]=1/0}function f(a,b){var c,d=[];for(c=0;c<b.length;c++)d[c]=a[b[c]];return d}function g(a){var b=a.split("|"),c=b[2].split(" "),g=b[3].split(""),h=b[4].split(" ");return d(c),d(g),d(h),e(h,g.length),{name:b[0],abbrs:f(b[1].split(" "),g),offsets:f(c,g),untils:h,population:0|b[5]}}
function h(a){a&&this._set(g(a))}function i(a){var b=a.toTimeString(),c=b.match(/\([a-z ]+\)/i);c&&c[0]?(c=c[0].match(/[A-Z]/g),c=c?c.join(""):void 0):(c=b.match(/[A-Z]{3,5}/g),c=c?c[0]:void 0),"GMT"===c&&(c=void 0),this.at=+a,this.abbr=c,this.offset=a.getTimezoneOffset()}function j(a){this.zone=a,this.offsetScore=0,this.abbrScore=0}function k(a,b){for(var c,d;d=6e4*((b.at-a.at)/12e4|0);)c=new i(new Date(a.at+d)),c.offset===a.offset?a=c:b=c;return a}function l(){var a,b,c,d=(new Date).getFullYear()-2,e=new i(new Date(d,0,1)),f=[e];for(c=1;c<48;c++)b=new i(new Date(d,c,1)),b.offset!==e.offset&&(a=k(e,b),f.push(a),f.push(new i(new Date(a.at+6e4)))),e=b;for(c=0;c<4;c++)f.push(new i(new Date(d+c,0,1))),
f.push(new i(new Date(d+c,6,1)));return f}function m(a,b){return a.offsetScore!==b.offsetScore?a.offsetScore-b.offsetScore:a.abbrScore!==b.abbrScore?a.abbrScore-b.abbrScore:b.zone.population-a.zone.population}function n(a,b){var c,e;for(d(b),c=0;c<b.length;c++)e=b[c],I[e]=I[e]||{},I[e][a]=!0}function o(a){var b,c,d,e=a.length,f={},g=[];for(b=0;b<e;b++){d=I[a[b].offset]||{};for(c in d)d.hasOwnProperty(c)&&(f[c]=!0)}for(b in f)f.hasOwnProperty(b)&&g.push(H[b]);return g}function p(){try{var a=Intl.DateTimeFormat().resolvedOptions().timeZone;if(a){var b=H[r(a)];if(b)return b;z("Moment Timezone found "+a+" from the Intl api, but did not have that data loaded.")}}catch(c){}
var d,e,f,g=l(),h=g.length,i=o(g),k=[];for(e=0;e<i.length;e++){for(d=new j(t(i[e]),h),f=0;f<h;f++)d.scoreOffsetAt(g[f]);k.push(d)}return k.sort(m),k.length>0?k[0].zone.name:void 0}function q(a){return D&&!a||(D=p()),D}function r(a){return(a||"").toLowerCase().replace(/\//g,"_")}function s(a){var b,c,d,e;for("string"==typeof a&&(a=[a]),b=0;b<a.length;b++)d=a[b].split("|"),c=d[0],e=r(c),F[e]=a[b],H[e]=c,d[5]&&n(e,d[2].split(" "))}function t(a,b){a=r(a);var c,d=F[a];return d instanceof h?d:"string"==typeof d?(d=new h(d),F[a]=d,d):G[a]&&b!==t&&(c=t(G[a],t))?(d=F[a]=new h,d._set(c),d.name=H[a],d):null}function u(){var a,b=[];for(a in H)H.hasOwnProperty(a)&&(F[a]||F[G[a]])&&H[a]&&b.push(H[a]);
