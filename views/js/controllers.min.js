...
}),define("taoClientDiagnostic/tools/performances/tester",["jquery","lodash","async","context","helpers","taoClientDiagnostic/tools/stats","taoQtiItem/qtiItem/core/Loader","taoQtiItem/qtiCommonRenderer/renderers/Renderer","lib/polyfill/performance-now"],function($,_,async,context,helpers,stats,Loader,Renderer){"use strict";function loadItem(data,done){var qtiJsonFile=data.url+"qti.json",urlTokens=data.url.split("/"),extension=urlTokens[0],fullpath=require.s.contexts._.config.paths[extension],baseUrl=data.url.replace(extension,fullpath),loader=new Loader,renderer=new Renderer({baseUrl:baseUrl});renderer.getAssetManager&&renderer.getAssetManager().setData("baseUrl",baseUrl),
require(["json!"+qtiJsonFile],function(itemData){loader.loadItemData(itemData,function(item){renderer.load(function(){var $container,duration,start,end,result;start=window.performance.now(),item.setRenderer(this),$container=$("<div>").appendTo("body"),$container.append(item.render()),item.postRender(),$container.remove(),end=window.performance.now(),duration=(end-start)/_second,result={id:data.id,url:data.url,duration:duration},done(null,result)},this.getLoadedClasses())})})}var _second=1e3,_defaultTimeout=30*_second,_defaultOccurrencesCount=10,_defaultSamples=["taoClientDiagnostic/tools/performances/data/sample1/","taoClientDiagnostic/tools/performances/data/sample2/","taoClientDiagnostic/tools/performances/data/sample3/"],_sampleBaseId="sample",performancesTester=function(samples,occurrences,timeout){
...
button:__("Begin diagnostics"),actionStore:"storeData",actionCheck:"check",controller:"DiagnosticChecker",extension:"taoClientDiagnostic"},_defaultsBrowser={action:"whichBrowser",controller:"CompatibilityChecker",extension:"taoClientDiagnostic"},_defaultsBandwidth={unit:.16,ideal:45,max:100},_defaultsPerformances={optimal:.025,threshold:.25},diagnostic={changeStatus:function(status){return this.is("rendered")&&this.controls.$status.html(status),this},store:function(type,details,done){var config=this.config;details=_.omit(details,"values"),details.type=type,$.post(helpers._url(config.actionStore,config.controller,config.extension,config.storeParams),details,done,"json");
},checkBrowser:function(done){var self=this,config=this.config;this.changeStatus(__("Checking the browser...")),browserTester(window,getConfig(this.config.browser,_defaultsBrowser)).start(function(information){$.post(helpers._url(config.actionCheck,config.controller,config.extension,config.storeParams),information,function(data){var percentage="success"===data.type?100:"warning"===data.type?33:0,status=self.status.getStatus(percentage,data),summary={browser:{message:__("Web browser"),value:information.browser+" "+information.browserVersion},os:{message:__("Operating system"),value:information.os+" "+information.osVersion}};status.id="browser",status.title=__("Operating system and web browser"),
...
}function program3(depth0,data){var stack1,helper,buffer="";return buffer+="<p>",(helper=helpers.info)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.info,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+"</p>"}this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,options,buffer="",functionType="function",escapeExpression=this.escapeExpression,self=this,helperMissing=helpers.helperMissing;return buffer+='<div class="diagnostics-main-area">\r\n\r\n    <h1>',(helper=helpers.title)?stack1=helper.call(depth0,{hash:{},
data:data}):(helper=depth0&&depth0.title,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'</h1>\r\n\r\n    <div class="intro">\r\n        ',stack1=helpers.if.call(depth0,depth0&&depth0.header,{hash:{},inverse:self.noop,fn:self.program(1,program1,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+="\r\n        ",stack1=helpers.if.call(depth0,depth0&&depth0.info,{hash:{},inverse:self.noop,fn:self.program(3,program3,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+='\r\n        <p>\r\n            <label for="workstation">'+escapeExpression((helper=helpers.__||depth0&&depth0.__,
options={hash:{},data:data},helper?helper.call(depth0,"Workstation:",options):helperMissing.call(depth0,"__","Workstation:",options)))+'</label>\r\n            <input type="text" data-control="workstation" id="workstation" name="workstation" maxlength="64" placeholder="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},helper?helper.call(depth0,"Workstation name",options):helperMissing.call(depth0,"__","Workstation name",options)))+'" />\r\n        </p>\r\n    </div>\r\n\r\n    <div class="clearfix">\r\n        <button data-action="test-launcher" class="btn-info small rgt">',(helper=helpers.button)?stack1=helper.call(depth0,{
hash:{},data:data}):(helper=depth0&&depth0.button,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'</button>\r\n    </div>\r\n\r\n    <ul class="plain results"></ul>\r\n\r\n    <div class="status">\r\n        <h2></h2>\r\n    </div>\r\n\r\n</div>\r\n'})}),define("taoClientDiagnostic/controller/Diagnostic/diagnostic",["jquery","i18n","helpers","layout/loading-bar","ui/actionbar","ui/feedback","taoClientDiagnostic/tools/diagnostic/diagnostic","tpl!taoClientDiagnostic/templates/diagnostic/main"],function($,__,helpers,loadingBar,actionbar,feedback,diagnosticFactory,diagnosticTpl){"use strict";var cssScope=".diagnostic-runner";
loadingBar.start();var taoDiagnosticRunnerCtlr={start:function(){function installTester(workstation){diagnosticFactory(config).setTemplate(diagnosticTpl).on("render",function(){var self=this;this.controls.$workstation=this.getElement().find('[data-control="workstation"]').on("keypress",function(e){13===e.which&&(e.preventDefault(),self.run())}).val(workstation)}).on("start",function(){this.config.storeParams=this.config.storeParams||{},this.config.storeParams.workstation=this.controls.$workstation.val(),this.controls.$workstation.prop("disabled",!0),loadingBar.start()}).on("end",function(){this.controls.$workstation.removeProp("disabled"),loadingBar.stop()}).on("render",function(){
loadingBar.stop()}).render($list)}var $container=$(cssScope),extension=$container.data("extension")||"taoClientDiagnostic",$list=$container.find(".list"),$panel=$(".panel"),config=$container.data("config"),indexUrl=helpers._url("index","Diagnostic",extension),workstationUrl=helpers._url("workstation","DiagnosticChecker",extension),buttons=[];buttons.push({id:"back",icon:"step-backward",title:__("Return to the list"),label:__("List of readiness checks"),action:function(){window.location.href=indexUrl}}),window.self===window.top&&buttons.push({id:"exitButton",icon:"close",title:__("Exit"),label:__("Exit"),action:function(){window.self.close()}}),actionbar({renderTo:$panel,
buttons:buttons}),$.get(workstationUrl,"json").done(function(data){installTester(data&&data.workstation)}).fail(function(){feedback().error(__("Unable to get the workstation name!")),installTester()})}};return taoDiagnosticRunnerCtlr}),define("taoClientDiagnostic/controller/Diagnostic/index",["jquery","i18n","helpers","layout/loading-bar","util/encode","ui/feedback","ui/dialog","taoClientDiagnostic/tools/diagnostic/status","ui/datatable"],function($,__,helpers,loadingBar,encode,feedback,dialog,statusFactory){"use strict";function formatNumber(number,digits){var nb=void 0===digits?2:Math.max(0,parseInt(digits,10)),factor=Math.pow(10,nb)||1;return Math.round(number*factor)/factor;
}var cssScope=".diagnostic-index";loadingBar.start();var taoDiagnosticCtlr={start:function(){function request(url,selection,message){selection&&selection.length&&(loadingBar.start(),$.ajax({url:url,data:{id:selection},dataType:"json",type:"POST",error:function(){loadingBar.stop()}}).done(function(response){loadingBar.stop(),response&&response.success?(message&&feedback().success(message),$list.datatable("refresh")):feedback().error(__("Something went wrong ...")+"<br>"+encode.html(response.error),{encodeHtml:!1})}))}function remove(selection){request(removeUrl,selection,__("The readiness check result have been removed"))}var $container=$(cssScope),extension=$container.data("extension")||"taoClientDiagnostic",$list=$container.find(".list"),dataset=$container.data("set"),config=$container.data("config")||{},installedExtension=$container.data("installedextension")||!1,diagnosticUrl=helpers._url("diagnostic","Diagnostic",extension),removeUrl=helpers._url("remove","Diagnostic",extension),serviceUrl=helpers._url("diagnosticData","Diagnostic",extension),performancesConfig=config.performances||{},performancesOptimal=performancesConfig.optimal,performancesRange=Math.abs(performancesOptimal-performancesConfig.threshold),diagnosticStatus=statusFactory(),tools=[],actions=[],model=[];
