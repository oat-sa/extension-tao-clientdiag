...
"use strict";var _kilo=1024,_mega=_kilo*_kilo,data=[],generateStr=function(length){for(var text="",possible="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789",i=0;length>i;i++)text+=possible.charAt(Math.floor(Math.random()*possible.length));return text},upload=function(size){var url=urlHelper.route("upload","CompatibilityChecker","taoClientDiagnostic",{cache:Date.now()}),str=generateStr(size);return data=[],$.ajax({url:url,type:"POST",data:{upload:str},xhr:function(){var xhr=new window.XMLHttpRequest,startTime=Date.now();return xhr.upload.addEventListener("progress",function(evt){if(evt.lengthComputable){var passedTime=Date.now()-startTime;data.push({
