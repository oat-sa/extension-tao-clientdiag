define("taoClientDiagnostic/controller/routes",[],function(){return{CompatibilityChecker:{actions:{index:"controller/compatibilityChecker/index"}}}}),define("taoClientDiagnostic/tools/fixedDecimals",[],function(){var fixedDecimals=function(value,decimals){var shift=Math.pow(10,Math.abs(decimals||1));return Math.round(Number(value)*shift)/shift||0};return fixedDecimals}),define("taoClientDiagnostic/tools/stats",["lodash","taoClientDiagnostic/tools/fixedDecimals"],function(_,fixedDecimals){var stats=function(list,fieldName,decimals){var variance,avg,med,middle,getValue,results,sum=0,sum2=0,min=Number.MAX_VALUE,max=0,count=0,values=[];return getValue=_.isFunction(fieldName)?fieldName:function(item){return item&&item[fieldName]||0
...
var tmp=r[k];r[k]=r[n],r[n]=tmp}return r},serialToIdentifier:function(interaction,choiceSerial){var choice=interaction.getChoice(choiceSerial);return choice?choice.id():""}}}),define("taoQtiItem/qtiRunner/core/Renderer",["lodash","jquery","handlebars","taoQtiItem/qtiItem/core/Element","taoQtiItem/qtiItem/helper/interactionHelper","ui/themeLoader"],function(_,$,Handlebars,Element,interactionHelper,themeLoader){function getDocumentBaseUrl(){return window.location.protocol+"//"+window.location.host+window.location.pathname.replace(/([^\/]*)$/,"")}var _isValidRenderer=function(renderer){var valid=!0;if("object"!=typeof renderer)return!1;var classCorrect=!1;if(renderer.qtiClass)if(_.indexOf(_renderableClasses,renderer.qtiClass)>=0)classCorrect=!0;
...
return{name:"commonRenderer",locations:locations,options:{assetManager:assetManager,themes:itemThemes}}}),define("taoQtiItem/qtiCommonRenderer/renderers/Renderer",["taoQtiItem/qtiRunner/core/Renderer","taoQtiItem/qtiCommonRenderer/renderers/config"],function(Renderer,config){return Renderer.build(config.locations,config.name,config.options)}),define("lib/polyfill/performance-now",[],function(){if("performance"in window||(window.performance={}),Date.now=Date.now||function(){return(new Date).getTime()},!("now"in window.performance)){var nowOffset=Date.now();window.performance.timing&&window.performance.timing.navigationStart&&(nowOffset=window.performance.timing.navigationStart),window.performance.now=function(){return Date.now()-nowOffset
}}}),define("taoClientDiagnostic/tools/performances/tester",["jquery","lodash","async","context","helpers","taoClientDiagnostic/tools/stats","taoQtiItem/qtiItem/core/Loader","taoQtiItem/qtiCommonRenderer/renderers/Renderer","lib/polyfill/performance-now"],function($,_,async,context,helpers,stats,Loader,Renderer){function loadItem(data,done){var qtiJsonFile=data.url+"qti.json",urlTokens=data.url.split("/"),extension=urlTokens[0],fullpath=require.s.contexts._.config.paths[extension],baseUrl=data.url.replace(extension,fullpath),loader=new Loader,renderer=new Renderer({baseUrl:baseUrl});renderer.getAssetManager&&renderer.getAssetManager().setData("baseUrl",baseUrl),require(["json!"+qtiJsonFile],function(itemData){loader.loadItemData(itemData,function(item){renderer.load(function(){var $container,duration,start,end,result;
start=window.performance.now(),item.setRenderer(this),$container=$("<div>").appendTo("body"),$container.append(item.render()),item.postRender(),$container.remove(),end=window.performance.now(),duration=(end-start)/_second,result={id:data.id,url:data.url,duration:duration},done(null,result)},this.getLoadedClasses())})})}var _second=1e3,_samples={sample1:{id:"sample1",url:"taoClientDiagnostic/tools/performances/data/sample1/",timeout:30*_second,nb:11},sample2:{id:"sample2",url:"taoClientDiagnostic/tools/performances/data/sample2/",timeout:30*_second,nb:10},sample3:{id:"sample3",url:"taoClientDiagnostic/tools/performances/data/sample3/",timeout:30*_second,nb:10}},performancesTester=function(){return{start:function(done){var tests=[];
_.forEach(_samples,function(data){for(var cb=_.partial(loadItem,data),iterations=data.nb||1;iterations--;)tests.push(cb)}),async.series(tests,function(err,measures){var results,decimals=2;if(err&&!measures.length)throw err;measures.shift(),results=stats(measures,"duration",decimals),done(results.average,results)})}}};return performancesTester}),define("taoClientDiagnostic/tools/bandwidth/tester",["lodash","async","context","taoClientDiagnostic/tools/stats","taoClientDiagnostic/tools/fixedDecimals","lib/polyfill/performance-now"],function(_,async,context,stats,fixedDecimals){var _kilo=1024,_mega=_kilo*_kilo,_second=1e3,_downloadData={"10KB":{id:"10KB",file:"data/bin10KB.data",size:10*_kilo,timeout:_second,threshold:0,nb:10},"100KB":{id:"100KB",file:"data/bin100KB.data",size:100*_kilo,timeout:2*_second,threshold:0,nb:5},"1MB":{id:"1MB",file:"data/bin1MB.data",size:_mega,timeout:20*_second,threshold:0,nb:3},"10MB":{id:"10MB",file:"data/bin10MB.data",size:10*_mega,timeout:20*_second,threshold:8,nb:3},"100MB":{id:"100MB",file:"data/bin100MB.data",size:100*_mega,timeout:60*_second,threshold:20,nb:3}},download=function(data,cb){var start,end,timeoutId,url,request,self=this;
return data.threshold&&this.bandwidth<data.threshold?cb("threshold"):(url=context.root_url+"/taoClientDiagnostic/views/js/tools/bandwidth/"+data.file+"?"+Date.now(),request=new XMLHttpRequest,request.open("GET",url,!0),request.setRequestHeader("Accept","application/octet-stream"),request.onload=function(){var duration,bytes,seconds,speed;return end=window.performance.now(),clearTimeout(timeoutId),duration=end-start,bytes=data.size,seconds=duration/_second,speed=8*bytes/seconds/_mega,self.bandwidth=Math.max(self.bandwidth,speed),cb(null,{id:data.id,file:data.file,size:data.size,duration:duration,speed:speed})},request.onerror=function(err){clearTimeout(timeoutId),cb(err)
},timeoutId=_.delay(cb,data.timeout,"timeout"),start=window.performance.now(),void request.send())},bandwidthTester=function(){return{start:function(done){var self=this,tests=[];_.forEach(_downloadData,function(data){for(var cb=_.bind(download,self,data),iterations=data.nb||1;iterations--;)tests.push(cb)}),this.bandwidth=0,async.series(tests,function(err,measures){var getValue,results,duration=0,size=0,decimals=2;if(err&&!measures.length)throw err;getValue=function(value){var speed=0;return value&&(duration+=value.duration,size+=value.size,speed=value.speed,value.speed=fixedDecimals(speed,decimals)),speed},results=stats(measures,getValue,decimals),results.duration=fixedDecimals(duration/_second,decimals),results.size=size,done(results.average,results)
})}}};return bandwidthTester}),define("taoClientDiagnostic/controller/CompatibilityChecker/diagnostics",["jquery","lodash","i18n","async","helpers","layout/loading-bar","taoClientDiagnostic/tools/performances/tester","taoClientDiagnostic/tools/bandwidth/tester","ui/feedback"],function($,_,__,async,helpers,loadingBar,performancesTester,bandwidthTester,feedback){function displayTestResult(name,status){var $bar=$('[data-result="'+name+'"]'),$indicator=$bar.find(".quality-indicator");$bar.find(".feedback").removeClass("feedback-error feedback-warning feedback-success").addClass("feedback-"+status.type).find(".msg").text(status.message),$bar.find(".icon").removeClass("icon-error icon-warning icon-success").addClass("icon-"+status.type),(_.isNumber(status.label)||_.isString(status.label))&&($indicator.attr("title",status.label),$indicator.toggleClass("wide",status.label.toString().length>2)),$bar.fadeIn(function(){$indicator.length&&$indicator.animate({left:status.percentage*$bar.outerWidth()/100-$indicator.outerWidth()/2})
})}function updateTestResult(name,status,score){displayTestResult(name,status),score&&(score[name]=status)}function displaySummary(score){var total=_.min(score,"percentage");displayTestResult("total",getStatus(summaryThresholds,total.percentage))}function checkBrowser(done){var info=new WhichBrowser,browser=info.browser,os=info.os,information={browser:browser&&browser.name||__("Unknown browser"),browserVersion:browser&&browser.version&&browser.version.original||__("Unknown version"),os:os&&os.name||__("Unknown OS"),osVersion:os&&os.version&&(os.version.alias||os.version.original)||__("Unknown version")};$.post(helpers._url("check","CompatibilityChecker","taoClientDiagnostic"),information,function(data){data.percentage="success"===data.type?100:0,done(data,information)
},"json")}function storeData(type,details,done){details=_.omit(details,"values"),details.type=type,$.post(helpers._url("storeData","CompatibilityChecker","taoClientDiagnostic"),details,done,"json")}function checkBandwidth(done){bandwidthTester().start(function(average,details){storeData("bandwidth",details,function(){var status=[];_.forEach(testTakers,function(threshold){var max=threshold*bandwidthUnit,st=getStatus(bandwidthThresholds,details.max/max*100),nb=Math.floor(details.max/bandwidthUnit);nb>maxTestTakers&&(nb=">"+maxTestTakers),st.label=nb,status.push(st)}),done(status,details)})})}function checkPerformance(done){performancesTester().start(function(average,details){var cursor=performanceRange-average+performanceOptimal,status=getStatus(thresholds,cursor/performanceRange*100);
storeData("performance",details,function(){done(status,details)})})}function displayDetails(information){var $detailsTable=$("#details");$("tbody",$detailsTable).empty(),$.each(information,function(index,object){var line="<td>"+object.message+"</td><td>"+object.value+"</td>";$("tbody",$detailsTable).append("<tr>"+line+"</tr>")})}var bandwidthUnit=.16,testTakers=[45],maxTestTakers=100,performanceOptimal=.025,performanceThreshold=.25,performanceRange=Math.abs(performanceOptimal-performanceThreshold),thresholds=[{threshold:0,message:__("Very slow performance"),type:"error"},{threshold:33,message:__("Average performance"),type:"warning"},{threshold:66,message:__("Good performance"),type:"success"}],bandwidthThresholds=[{threshold:0,message:__("Low bandwidth"),type:"error"},{threshold:33,message:__("Average bandwidth"),type:"warning"},{threshold:66,message:__("Good bandwidth"),type:"success"}],summaryThresholds=[{threshold:0,message:__("Your system requires a compatibility update, please contact your system administrator."),type:"error"},{threshold:33,message:__("Your system is not optimal, please contact your system administrator."),type:"warning"},{threshold:66,message:__("Your system is fully compliant."),type:"success"}],getStatus=function(thresholds,percentage){var i,step,len=thresholds.length,status=thresholds[0];
for(percentage=Math.max(0,Math.min(100,Math.round(percentage))),i=1;len>i&&(step=thresholds[i],step&&percentage>=step.threshold);i++)status=step;return status=_.clone(status),status.percentage=percentage,status},init=function(){var status,$testTriggerBtn=$('[data-action="test-launcher"]'),$detailsBtn=$('[data-action="display-details"]'),information={},scores={},$feedbackBox=$("#feedback-box");$testTriggerBtn.on("click",function(){loadingBar.start(),$testTriggerBtn.hide(),async.series([function(cb){checkBrowser(function(status,details){_.assign(information,{browser:{message:__("Web browser"),value:details.browser+" "+details.browserVersion},os:{message:__("Operating system"),value:details.os+" "+details.osVersion}}),displayDetails(information),updateTestResult("browser",status,scores),cb()
})},function(cb){checkPerformance(function(status,details){_.assign(information,{performancesMin:{message:__("Minimum rendering time"),value:details.min+" s"},performancesMax:{message:__("Maximum rendering time"),value:details.max+" s"},performancesAverage:{message:__("Average rendering time"),value:details.average+" s"}}),displayDetails(information),updateTestResult("performance",status,scores),cb()})},function(cb){checkBandwidth(function(status,details){_.assign(information,{bandwidthMin:{message:__("Minimum bandwidth"),value:details.min+" Mbps"},bandwidthMax:{message:__("Maximum bandwidth"),value:details.max+" Mbps"},bandwidthAverage:{message:__("Average bandwidth"),value:details.average+" Mbps"}}),displayDetails(information),_.forEach(status,function(st,i){updateTestResult("bandwidth-"+i,st,scores)
}),cb()})}],function(){displaySummary(scores),loadingBar.stop()})}),$detailsBtn.on("click",function(){loadingBar.start(),$detailsBtn.hide(),displayDetails(information),status=getStatus(thresholds,68),displayTestResult("details",status),loadingBar.stop()}),$feedbackBox.data("error")&&feedback().error($feedbackBox.data("error")),$feedbackBox.data("message")&&feedback().error($feedbackBox.data("message"))};return init()});
