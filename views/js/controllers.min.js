...
}),define("lib/polyfill/performance-now",[],function(){"use strict";if("performance"in window||(window.performance={}),Date.now=Date.now||function(){return(new Date).getTime()},!("now"in window.performance)){var nowOffset=Date.now();window.performance.timing&&window.performance.timing.navigationStart&&(nowOffset=window.performance.timing.navigationStart),window.performance.now=function(){return Date.now()-nowOffset}}}),define("taoClientDiagnostic/tools/performances/tester",["jquery","lodash","async","context","helpers","taoClientDiagnostic/tools/stats","taoQtiItem/qtiItem/core/Loader","taoQtiItem/qtiCommonRenderer/renderers/Renderer","lib/polyfill/performance-now"],function($,_,async,context,helpers,stats,Loader,Renderer){
...
var self=this,tests=[];_.forEach(_downloadData,function(data){for(var cb=_.bind(download,self,data),iterations=data.nb||1;iterations--;)tests.push(cb)}),this.bandwidth=0,async.series(tests,function(err,measures){var getValue,results,duration=0,size=0,decimals=2;if(err&&!measures.length)throw err;getValue=function(value){var speed=0;return value&&(duration+=value.duration,size+=value.size,speed=value.speed,value.speed=fixedDecimals(speed,decimals)),speed},results=stats(measures,getValue,decimals),results.duration=fixedDecimals(duration/_second,decimals),results.size=size,done(results.average,results)})}}};return bandwidthTester}),define("taoClientDiagnostic/controller/CompatibilityChecker/diagnostics",["jquery","lodash","i18n","async","helpers","layout/loading-bar","taoClientDiagnostic/tools/performances/tester","taoClientDiagnostic/tools/bandwidth/tester","ui/feedback"],function($,_,__,async,helpers,loadingBar,performancesTester,bandwidthTester,feedback){
"use strict";function displayTestResult(name,status){var $bar=$('[data-result="'+name+'"]'),$indicator=$bar.find(".quality-indicator");$bar.find(".feedback").removeClass("feedback-error feedback-warning feedback-success").addClass("feedback-"+status.type).find(".msg").text(status.message),$bar.find(".icon").removeClass("icon-error icon-warning icon-success").addClass("icon-"+status.type),(_.isNumber(status.label)||_.isString(status.label))&&($indicator.attr("title",status.label),$indicator.toggleClass("wide",status.label.toString().length>2)),$bar.fadeIn(function(){$indicator.length&&$indicator.animate({left:status.percentage*$bar.outerWidth()/100-$indicator.outerWidth()/2
})})}function updateTestResult(name,status,score){displayTestResult(name,status),score&&(score[name]=status)}function displaySummary(score){var total=_.min(score,"percentage");displayTestResult("total",getStatus(summaryThresholds,total.percentage))}function checkBrowser(done){var info=new WhichBrowser,browser=info.browser,os=info.os,information={browser:browser&&browser.name||__("Unknown browser"),browserVersion:browser&&browser.version&&browser.version.original||__("Unknown version"),os:os&&os.name||__("Unknown OS"),osVersion:os&&os.version&&(os.version.alias||os.version.original)||__("Unknown version")};$.post(helpers._url("check","CompatibilityChecker","taoClientDiagnostic"),information,function(data){
"success"===data.type?data.percentage=100:data.percentage=0,done(data,information)},"json")}function storeData(type,details,done){details=_.omit(details,"values"),details.type=type,$.post(helpers._url("storeData","CompatibilityChecker","taoClientDiagnostic"),details,done,"json")}function checkBandwidth(config,done){var unit=config.unit||bandwidthUnit,tt=config.ideal||testTakers,maxTT=config.max||maxTestTakers;_.isArray(tt)||(tt=[tt]),bandwidthTester().start(function(average,details){storeData("bandwidth",details,function(){var status=[];_.forEach(tt,function(threshold){var max=threshold*unit,st=getStatus(bandwidthThresholds,details.max/max*100),nb=Math.floor(details.max/unit);
nb>maxTT&&(nb=">"+maxTT),st.label=nb,status.push(st)}),done(status,details)})})}function checkPerformance(config,done){var optimal=config.optimal||performanceOptimal,range=Math.abs(optimal-(config.threshold||performanceThreshold));performancesTester(config.samples,config.occurrences,1e3*config.timeout).start(function(average,details){var cursor=range-average+optimal,status=getStatus(thresholds,cursor/range*100);storeData("performance",details,function(){done(status,details)})})}function displayDetails(information){var $detailsTable=$("#details");$("tbody",$detailsTable).empty(),$.each(information,function(index,object){var line="<td>"+object.message+"</td><td>"+object.value+"</td>";
$("tbody",$detailsTable).append("<tr>"+line+"</tr>")})}var bandwidthUnit=.16,testTakers=[45],maxTestTakers=100,performanceOptimal=.025,performanceThreshold=.25,thresholds=[{threshold:0,message:__("Very slow performance"),type:"error"},{threshold:33,message:__("Average performance"),type:"warning"},{threshold:66,message:__("Good performance"),type:"success"}],bandwidthThresholds=[{threshold:0,message:__("Low bandwidth"),type:"error"},{threshold:33,message:__("Average bandwidth"),type:"warning"},{threshold:66,message:__("Good bandwidth"),type:"success"}],summaryThresholds=[{threshold:0,message:__("Your system requires a compatibility update, please contact your system administrator."),
type:"error"},{threshold:33,message:__("Your system is not optimal, please contact your system administrator."),type:"warning"},{threshold:66,message:__("Your system is fully compliant."),type:"success"}],getStatus=function(thresholds,percentage){var i,step,len=thresholds.length,status=thresholds[0];for(percentage=Math.max(0,Math.min(100,Math.round(percentage))),i=1;len>i&&(step=thresholds[i],step&&percentage>=step.threshold);i++)status=step;return status=_.clone(status),status.percentage=percentage,status},getResultConfig=function(name){var $result=$('[data-result="'+name+'"]'),config=$result.data("config");return config||{}},init=function(){var status,$testTriggerBtn=$('[data-action="test-launcher"]'),$detailsBtn=$('[data-action="display-details"]'),information={},scores={},$feedbackBox=$("#feedback-box");
$testTriggerBtn.on("click",function(){loadingBar.start(),$testTriggerBtn.hide(),async.series([function(cb){checkBrowser(function(status,details){_.assign(information,{browser:{message:__("Web browser"),value:details.browser+" "+details.browserVersion},os:{message:__("Operating system"),value:details.os+" "+details.osVersion}}),displayDetails(information),updateTestResult("browser",status,scores),cb()})},function(cb){checkPerformance(getResultConfig("performance"),function(status,details){_.assign(information,{performancesMin:{message:__("Minimum rendering time"),value:details.min+" s"},performancesMax:{message:__("Maximum rendering time"),value:details.max+" s"
},performancesAverage:{message:__("Average rendering time"),value:details.average+" s"}}),displayDetails(information),updateTestResult("performance",status,scores),cb()})},function(cb){checkBandwidth(getResultConfig("bandwidth-0"),function(status,details){_.assign(information,{bandwidthMin:{message:__("Minimum bandwidth"),value:details.min+" Mbps"},bandwidthMax:{message:__("Maximum bandwidth"),value:details.max+" Mbps"},bandwidthAverage:{message:__("Average bandwidth"),value:details.average+" Mbps"}}),displayDetails(information),_.forEach(status,function(st,i){updateTestResult("bandwidth-"+i,st,scores)}),cb()})}],function(){displaySummary(scores),loadingBar.stop();
