...
Element.isA(element,"interaction")?this.loadInteractionData(element,data):Element.isA(element,"choice")?this.loadChoiceData(element,data):Element.isA(element,"math")?this.loadMathData(element,data):Element.isA(element,"infoControl")&&this.loadPicData(element,data),element},loadInteractionData:function(interaction,data){Element.isA(interaction,"blockInteraction")&&data.prompt&&this.loadContainer(interaction.prompt.getBody(),data.prompt),this.buildInteractionChoices(interaction,data),Element.isA(interaction,"customInteraction")&&this.loadPciData(interaction,data)},buildInteractionChoices:function(interaction,data){if(data.choices){if(Element.isA(interaction,"matchInteraction"))for(var set=0;2>set;set++){
if(!data.choices[set])throw"missing match set #"+set;var matchSet=data.choices[set];for(var serial in matchSet){var choice=this.buildElement(matchSet[serial]);choice&&interaction.addChoice(choice,set)}}else for(var serial in data.choices){var choice=this.buildElement(data.choices[serial]);choice&&interaction.addChoice(choice)}if(Element.isA(interaction,"graphicGapMatchInteraction")&&data.gapImgs)for(var serial in data.gapImgs){var gapImg=this.buildElement(data.gapImgs[serial]);gapImg&&interaction.addGapImg(gapImg)}}},loadChoiceData:function(choice,data){Element.isA(choice,"textVariableChoice")?choice.val(data.text):Element.isA(choice,"gapImg")||Element.isA(choice,"containerChoice");
...
hash:{},inverse:self.noop,fn:self.program(1,program1,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+="\n\n        ",stack1=helpers["with"].call(depth0,depth0&&depth0.quality,{hash:{},inverse:self.noop,fn:self.program(4,program4,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+="\n    </div>\n    ",stack1=helpers["with"].call(depth0,depth0&&depth0.details,{hash:{},inverse:self.noop,fn:self.program(9,program9,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+="\n</li>\n"})}),define("css!taoClientDiagnosticCss/diagnostics",[],function(){}),define("taoClientDiagnostic/tools/diagnostic/diagnostic",["jquery","lodash","i18n","async","helpers","ui/feedback","ui/component","taoClientDiagnostic/tools/performances/tester","taoClientDiagnostic/tools/bandwidth/tester","taoClientDiagnostic/tools/browser/tester","taoClientDiagnostic/tools/getconfig","tpl!taoClientDiagnostic/tools/diagnostic/tpl/main","tpl!taoClientDiagnostic/tools/diagnostic/tpl/result","css!taoClientDiagnosticCss/diagnostics"],function($,_,__,async,helpers,feedback,component,performancesTester,bandwidthTester,browserTester,getConfig,mainTpl,resultTpl){
"use strict";function getStatus(percentage,thresholds){var len,feedback,i,step,status;if(percentage=Math.max(0,Math.min(100,Math.round(parseInt(percentage,10)))),thresholds)for(_.isArray(thresholds)||(thresholds=[thresholds]),len=thresholds.length,feedback=thresholds[0],i=1;len>i&&(step=thresholds[i],step&&percentage>=step.threshold);i++)feedback=step;return status={percentage:percentage,quality:{}},feedback&&(status.feedback=_.clone(feedback)),status}function diagnosticFactory(config){return component(diagnostic,_defaults).setTemplate(mainTpl).on("destroy",function(){this.controls=null}).on("render",function(){var self=this;this.controls={$start:this.$component.find('[data-action="test-launcher"]'),
$status:this.$component.find(".status"),$results:this.$component.find(".results")},this.controls.$start.on("click",function(){self.run()}),this.controls.$results.on("click",'button[data-action="show-details"]',function(){var $btn=$(this).closest("button"),$result=$btn.closest("[data-result]"),$details=$result.find(".details");$details.removeClass("hidden"),$btn.addClass("hidden"),$result.find('[data-action="hide-details"]').removeClass("hidden")}),this.controls.$results.on("click",'button[data-action="hide-details"]',function(){var $btn=$(this).closest("button"),$result=$btn.closest("[data-result]"),$details=$result.find(".details");$details.addClass("hidden"),
$btn.addClass("hidden"),$result.find('[data-action="show-details"]').removeClass("hidden")})}).init(config)}var _defaults={title:__("Diagnostic Tool"),header:__("This tool will run a number of tests in order to establish how well your current environment is suitable to run the TAO platform."),info:__("Be aware that these tests will take up to several minutes."),button:__("Begin diagnostics"),actionStore:"storeData",actionCheck:"check",controller:"CompatibilityChecker",extension:"taoClientDiagnostic"},_defaultsBrowser={action:"whichBrowser",controller:"CompatibilityChecker",extension:"taoClientDiagnostic"},_defaultsBandwidth={unit:.16,ideal:45,max:100},_defaultsPerformances={
optimal:.025,threshold:.25},performancesThresholds=[{threshold:0,message:__("Very slow performance"),type:"error"},{threshold:33,message:__("Average performance"),type:"warning"},{threshold:66,message:__("Good performance"),type:"success"}],bandwidthThresholds=[{threshold:0,message:__("Low bandwidth"),type:"error"},{threshold:33,message:__("Average bandwidth"),type:"warning"},{threshold:66,message:__("Good bandwidth"),type:"success"}],summaryThresholds=[{threshold:0,message:__("Your system requires a compatibility update, please contact your system administrator."),type:"error"},{threshold:33,message:__("Your system is not optimal, please contact your system administrator."),
type:"warning"},{threshold:66,message:__("Your system is fully compliant."),type:"success"}],diagnostic={changeStatus:function(status){return this.is("rendered")&&this.controls.$status.html(status),this},store:function(type,details,done){var config=this.config;details=_.omit(details,"values"),details.type=type,$.post(helpers._url(config.actionStore,config.controller,config.extension),details,done,"json")},checkBrowser:function(done){var self=this,config=this.config;this.changeStatus(__("Checking the browser...")),browserTester(window,getConfig(this.config.browser,_defaultsBrowser)).start(function(information){$.post(helpers._url(config.actionCheck,config.controller,config.extension),information,function(data){
var percentage="success"===data.type?100:0,status=getStatus(percentage,data),summary={browser:{message:__("Web browser"),value:information.browser+" "+information.browserVersion},os:{message:__("Operating system"),value:information.os+" "+information.osVersion}};status.id="browser",status.title=__("Operating system and web browser"),self.addResult(status),done(status,summary)},"json")})},checkPerformances:function(done){var self=this,config=getConfig(this.config.performances,_defaultsPerformances),optimal=config.optimal,range=Math.abs(optimal-config.threshold);this.changeStatus(__("Checking the performances...")),performancesTester(config.samples,config.occurrences,1e3*config.timeout).start(function(average,details){
var cursor=range-average+optimal,status=getStatus(cursor/range*100,performancesThresholds),summary={performancesMin:{message:__("Minimum rendering time"),value:details.min+" s"},performancesMax:{message:__("Maximum rendering time"),value:details.max+" s"},performancesAverage:{message:__("Average rendering time"),value:details.average+" s"}};self.store("performance",details,function(){status.id="performance",status.title=__("Workstation performance"),self.addResult(status),done(status,summary)})})},checkBandwidth:function(done){var self=this,config=getConfig(this.config.bandwidth,_defaultsBandwidth);this.changeStatus(__("Checking the bandwidth...")),bandwidthTester().start(function(average,details){
var summary={bandwidthMin:{message:__("Minimum bandwidth"),value:details.min+" Mbps"},bandwidthMax:{message:__("Maximum bandwidth"),value:details.max+" Mbps"},bandwidthAverage:{message:__("Average bandwidth"),value:details.average+" Mbps"}};self.store("bandwidth",details,function(){var status=[],bandwidthUnit=config.unit,testTakers=config.ideal,maxTestTakers=config.max;_.isArray(testTakers)||(testTakers=[testTakers]),_.forEach(testTakers,function(threshold,i){var max=threshold*bandwidthUnit,st=getStatus(details.max/max*100,bandwidthThresholds),nb=Math.floor(details.max/bandwidthUnit);nb>maxTestTakers&&(nb=">"+maxTestTakers),st.id="bandwidth-"+i,st.title=__("Bandwidth"),
st.feedback.legend=__("Number of simultaneous test takers the connection can handle"),st.quality.label=nb,nb.toString().length>2&&(st.quality.wide=!0),status.push(st),self.addResult(st)}),done(status,summary)})})},addResult:function(result){var $result,$indicator;return this.is("rendered")&&(result.quality&&result.quality.label&&result.quality.label.toString().length>2&&(result.quality.wide=!0),$result=$(resultTpl(result)),$indicator=$result.find(".quality-indicator"),this.controls.$results.append($result),$result.fadeIn(function(){$indicator.length&&$indicator.animate({left:result.percentage*$result.outerWidth()/100-$indicator.outerWidth()/2})})),this},prepare:function(){
return this.trigger("start"),this.changeStatus(__("Starting...")),this.setState("running",!0),this.setState("done",!1),this.controls.$results.empty(),this.controls.$start.addClass("hidden"),this},finish:function(){return this.controls.$start.removeClass("hidden"),this.trigger("end"),this.changeStatus(__("Done!")),this.setState("running",!1),this.setState("done",!0),this},run:function(){function doCheck(method,cb){self.trigger("starttester",method),self.setState(method,!0),self[method](function(status,details){_.assign(information,details),_.isArray(status)||(status=[status]),_.forEach(status,function(st){scores[st.id]=st}),self.trigger("endtester",method,status),
self.setState(method,!1),cb()})}var self=this,information={},scores={};return this.is("rendered")&&(this.prepare(),async.series([function(cb){doCheck("checkBrowser",cb)},function(cb){doCheck("checkPerformances",cb)},function(cb){doCheck("checkBandwidth",cb)}],function(){var total=_.min(scores,"percentage"),status=getStatus(total.percentage,summaryThresholds);status.title=__("Total"),status.id="total",status.details=information,self.addResult(status),self.finish()})),this}};return diagnosticFactory}),define("taoClientDiagnostic/controller/CompatibilityChecker/diagnostics",["jquery","layout/loading-bar","taoClientDiagnostic/tools/diagnostic/diagnostic","ui/feedback"],function($,loadingBar,diagnosticFactory,feedback){
"use strict";return{start:function(config){var $feedbackBox=$("#feedback-box"),$contentArea=$(".content");$feedbackBox.data("error")&&feedback().error($feedbackBox.data("error")),$feedbackBox.data("message")&&feedback().error($feedbackBox.data("message")),diagnosticFactory(config).on("start",function(){loadingBar.start()}).on("end",function(){loadingBar.stop()}).render($contentArea)}}}),function(c){var d=document,a="appendChild",i="styleSheet",s=d.createElement("style");s.type="text/css",d.getElementsByTagName("head")[0][a](s),s[i]?s[i].cssText=c:s[a](d.createTextNode(c))}('.diagnostics-main-area{max-width:768px;margin:40px auto 0 auto;background:#f3f1ef;border:1px solid #ddd;border-radius:2px;-webkit-border-radius:2px;padding:20px}.diagnostics-main-area h1,.diagnostics-main-area h2,.diagnostics-main-area h3{font-style:normal;font-family:"Century Gothic",CenturyGothic,AppleGothic,sans-serif}.diagnostics-main-area h1{margin-bottom:20px;font-size:20px;font-size:2rem}.diagnostics-main-area h2{margin:0;font-size:14px;font-size:1.4rem}.diagnostics-main-area h3{margin-top:10px;margin-bottom:5px;font-size:12px;font-size:1.2rem}.diagnostics-main-area .status,.diagnostics-main-area .details h2{margin:1rem}.diagnostics-main-area .legend{font-style:italic;font-weight:normal;font-family:"Century Gothic",CenturyGothic,AppleGothic,sans-serif;font-size:11px;font-size:1.1rem}.diagnostics-main-area .quality-bar{height:15px;background:linear-gradient(to right, #CE1431 0%, #D57915 25%, #D5A215 50%, #D5CB15 75%, #36B111 100%);opacity:.8;-moz-border-radius:2px;-webkit-border-radius:2px;border-radius:2px;margin:5px 0 10px;position:relative}.diagnostics-main-area .quality-indicator{position:absolute;border-width:10px 8px;border-style:solid;border-color:#333 transparent transparent;top:-1px;height:25px;width:0}.diagnostics-main-area .quality-indicator[title]:before{font-size:11px;font-size:1.1rem;content:attr(title);position:absolute;top:-36px;left:-12px;text-align:center;width:24px;height:24px;line-height:24px;display:inline-block;background-color:#0e5d91;color:rgba(255,255,255,0.9);-moz-border-radius:12px;-webkit-border-radius:12px;border-radius:12px}.diagnostics-main-area .quality-indicator.wide[title]:before{left:-16px;width:32px}.diagnostics-main-area [data-result],.diagnostics-main-area .bandwidth-box{display:none}.diagnostics-main-area [data-action=bandwidth-launcher]{margin:5px 0 10px}.diagnostics-main-area li{padding-bottom:10px}.diagnostics-main-area .intro{margin-bottom:20px}.diagnostics-main-area table.matrix{border:1px solid #ddd;border-radius:2px;-webkit-border-radius:2px;margin-top:5px}\n/*# sourceMappingURL=diagnostics.css.map */\n');
