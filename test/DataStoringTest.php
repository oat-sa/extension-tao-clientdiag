    ...
    public function testStoreData(){
        ...
        if (($handle = fopen($this->sampleFilePath, "r")) !== FALSE) {
            while (($data = fgetcsv($handle, 1000, ";")) !== FALSE) {
                ...
                $row ++;
                if($row === 2){
                    $this->assertEquals($this->sentData['browser'],$data[0]);
                    $this->assertEquals($this->sentData['browserVersion'],$data[1]);
                    $this->assertEquals($this->sentData['ip'],$data[2]);
                    $this->assertEquals($this->sentData['key'],$data[3]);
                    $this->assertEquals($this->sentData['login'],$data[4]);
                    $this->assertEquals($this->sentData['os'],$data[5]);
                    $this->assertEquals($this->sentData['osVersion'],$data[6]);
                    $this->assertEquals(1,$data[7]);
                }
                if($row === 3){
                    $this->assertEquals(0,$data[7]);
                }
            }
            fclose($handle);
